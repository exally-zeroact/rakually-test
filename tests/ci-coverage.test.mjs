...
const EXCLUDED = {
  ...
  'kyuyo/tests/yameta-hito-mieru.mjs': {
    reason: '★辞めた 人が 自分の 給与明細を ★本人の 合言葉で 開けるか★を 実ブラウザで 見る 道具★'
      + '（司さん 2026-09-24「1 見れた方がええやろが」の 直しの 門）。'
      + '★支度で 倉庫に 合言葉(crypt)を 置き、後始末で 鍵を 消す★＝★倉庫の 管理の 鍵が 要る★。'
      + '★CI には その 鍵が 無い★ので、載せても ★毎回 未測定で 通るだけ★＝'
      + '★★「在るのに 0回」＝見張りでは ない★★（webkit.yml の 頭に 在る 決めと 同じ）。'
      + '★更に 悪い★＝後始末の `delete` が CI では 出来ず ★走る たび 鍵と 紙が 増える★'
      + '（★鍵は CASCADE で 紙を 道連れに するので 消せない★＝2026-09-22 の 決め）。'
      + '★手元では 緑★＝5 passed 0 failed／★--waza（前の 形に 戻す）で 赤に なる事も 確かめた★。',
    restoreWhen: '★CI に 倉庫の 管理の 鍵を 置いた日★、または ★倉庫を 触らずに 同じ 事を 測る 台を 作った日★。'
