...
T('★帯のぶんだけ中身を下げる（上の帯・1行目を隠さない）', () => {
  ...
  /* ★★被せ物（fixed）の 中の 貼り付きは 動かさない★★（2026-09-10 実測）
     帯は 画面の いちばん上。被せ物は その上に 全面を 覆うので ★中の 貼り付きは 隠れない★。
     なのに 下げると ★被せの 頭が 46px 下がって、すぐ下の ボタンが その裏に 隠れる★
     （請求書の「紙を見る」かぶせで 実際に そうなった＝PDF・印刷の ボタンの 上半分が 見えなかった）。 */
  ok(/fixedNaka/.test(src), '★被せ物（fixed）の 中の 貼り付きまで 下げている★');
  ok(/if \(fixedNaka\) continue;/.test(src), '★見つけても 飛ばしていない★');
});
