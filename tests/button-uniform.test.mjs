...
/* ★押せない時の 3色＝アプリの 警告と 同じ物★（`.sh-warn` `.cr-warn` `.cap-badge`） */
const WARN_BG = 'rgb(255, 248, 225)';   // #FFF8E1
const WARN_INK = 'rgb(146, 80, 10)';    // #92500A
const WARN_EDGE = 'rgb(244, 216, 168)'; // #F4D8A8
...
export const ALLOW = [
  ...
  /* ★★押せない（警告）★★（2026-09-25 司さん「★ルール通り 警告のときみたいに やれや★」）
     ★前★ … 押せない物は ★塗った 緑を 薄めた 顔★（#EAF3ED／#527A66／#DCE9E1）だった
       ＝★台帳に 無い 顔★／★「押せない」のか「地味なだけ」なのか 見分けが つかない★
     ★今★ … ★アプリの 警告と 同じ 3色★（`.sh-warn` `.cr-warn` `.cap-badge` と 同じ）
     ★訳は これまで どおり 字で 言う★＝札の 中に「県が未選択◯名」等（2026-09-05 指示役の 裁定）
     ★これは『押す物の 顔』では なく『今は 押せません』の 知らせ★＝★本命の 緑を 薄めない★ */
  { k: WARN_BG + ' | ' + WARN_INK + ' | ' + WARN_EDGE + ' 1.5px', why: '★押せない（警告）★＝地 #FFF8E1・字 #92500A・枠 #F4D8A8' },
];
