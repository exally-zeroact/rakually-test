...
(function (root, factory) {
  ...
  function setCell(sheetXml, addr, spec, book) {
    ...
    var t = '', body = '';
    if (spec.t === 's') {
      /* ★共有文字列の部品が 無いブックが ある★（他の道具が作った xlsx）。
         そこへ t="s" と 番号を書くと ★番号の指す先が 無い＝空のマス★になる
         （2026-08-31 実測：書けたと言うのに 読み返すと 空だった）。
         ⇒ ★部品が 無い時は そのマスの中に 字を そのまま入れる（inlineStr）★。 */
      if (book.sst && book.sst.xml) {
        var idx = addString(book, spec.v == null ? '' : String(spec.v));
        t = ' t="s"'; body = '<v>' + idx + '</v>';
      } else {
        t = ' t="inlineStr"';
        body = '<is><t xml:space="preserve">' + esc(spec.v == null ? '' : String(spec.v)) + '</t></is>';
      }
    } else if (spec.t === 'b') {
