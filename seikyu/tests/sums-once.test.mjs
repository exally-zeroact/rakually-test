/* sums-once.test.mjs — ★紙に「小計」は 1回だけ★（同じ言葉が 2回 出ない）
 * ============================================================================
 * ★なぜ（指示役 2026-09-02 が 絵を見て 見つけた／私が 実物で 数えた）★
 *   1枚物の紙に ★「小計」が 2回★ 出ている
 *     ①表の最終行（列の真下）  ②その下の 締め（小計の所）
 *   ★実物45枚（16社）では 「小計」は 0回★（機械で読んだ）。
 ...
 *   ・★言葉の直し（小計→小計）は 別の回★（指示役「同じ回に 混ぜない」＝
 *     実測 seikyu-paper.test.mjs だけで ★25か所★ が この言葉を 固定している）
 *
 * ★ここで見る事★
 *   ① 1枚物の紙で ★「小計」は 1回だけ★（表の中には 出ない）
 *   ② ★数は 消えていない★＝締めの「小計」の額 ＝ 明細の金額を 足した額
...
T('★② 数は 消えていない（締めの額＝明細を足した額）', () => {
  ...
  ok(t.indexOf(yen(tax.subtotal)) >= 0, '★小計の額（' + yen(tax.subtotal) + '）が 紙に 無い★');
  ok(t.indexOf(yen(tax.grandTotal)) >= 0, '★合計の額（' + yen(tax.grandTotal) + '）が 紙に 無い★');
  console.log('     額は 出ている … 小計 ' + yen(tax.subtotal) + ' ／ 合計 ' + yen(tax.grandTotal));
});
