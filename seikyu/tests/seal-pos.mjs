...
/* ═══ ★自社の塊は「ご請求金額」と 下をそろえる★ ═══
   ...
   ・★頭の余白（あて名の下〜◯月分の上 15.3mm）も 行数(★2026-09-08 から 16/5★)も 元のまま★
...
/* ★2026-09-08 18/8 → 17/7 に 直した★
   ★この見張りの 役目は 変わらない★＝「★印の 直しで 行数を 勝手に 詰めていない★」。
   ★数を 手で 書いてあるのは わざと★＝誰かが 動かしたら その日に 赤に なる。
   ★今回 動かした 訳★＝司さん「他のテンプレも…（項目の合計など）」
     →「は？控除なしのやつって言わんかったか？」で
     ★表の 一番下に「項目の合計」の 1行を 足した★ので、
     明細に 使える 行が 1つ 減る（減らさないと A4を 超える＝memo-waku が 捕まえた）。
   ★印の 話では 1行も 動かしていない★。 */
T('★⑨-4c 1枚に載る行数も 元のまま（16/5＝足元の備考の箱を消した後の 数）',
  PAPER.PAPER_ROWS === 16 && PAPER.PAPER_ROWS_DED === 5,
  '1枚に載る行数 ' + PAPER.PAPER_ROWS + '/' + PAPER.PAPER_ROWS_DED + '（16/5のはず）');
