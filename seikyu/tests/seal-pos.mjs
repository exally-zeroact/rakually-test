...
/* ═══ ★自社の塊は「ご請求金額」と 下をそろえる★ ═══
   ...
   ・★頭の余白（あて名の下〜◯月分の上 15.3mm）も 行数(★2026-09-08 から 16/7★)も 元のまま★
...
/* ★2026-09-08 18/8 → 17/7 に 直した★
   ...
   ★印の 話では 1行も 動かしていない★。 */
T('★⑨-4c 1枚に載る行数も 元のまま（16/7＝合計行＋帯を足した後の 数）',
  PAPER.PAPER_ROWS === 16 && PAPER.PAPER_ROWS_DED === 7,
  '1枚に載る行数 ' + PAPER.PAPER_ROWS + '/' + PAPER.PAPER_ROWS_DED + '（16/7のはず）');
