...
/* ★A4 1枚に載る数は「測った数」＝勝手に上げたら赤★
   ...
   ★紙に何かを足した日／詰めた日に この数は変わる★
     ...
     → ★20/10（2026-08-31 自社の塊を「ご請求金額」と同じ段に入れて 下をそろえた）★
       ＝自社(19.4mm)が あて名(7.2mm)の段の高さを 決めていたのを やめたので
         ★頭が 12.2mm 低くなり 明細が 2行 増えた★
       ★1行ずつ 総当たりで 測り直した★（WebKit・sheet の scrollHeight で はみ出しを見た）
         控除なし … 20行=余り0px ／ ★21行で +3px はみ出し★
         控除あり … 10行=余り0px ／ ★11行で +16px はみ出し★ */
...
T('★★途中の紙は目一杯 載せて 紙を増やさない（最後の紙だけ控除と締めの分 少なく）★★', () => {
  ...
  /* ★60行は 引き分けになった★（最後の枠が 18→20 になったので 昔の分け方でも 3枚）
     ＝★分け方が悪いのではなく 1枚に載る数が 増えたから★。
     見たいのは「途中の紙を目一杯 使うと 紙が減る」なので、
     ★差が出る本数（61行）★に 置き換えた（60行は 3枚のまま＝増えていない事だけ 下で見る）。 */
  const cases = [[26, 1], [40, 1], [61, 0], [100, 1]];
  cases.forEach(([n, ded]) => {
    const b = mk(n, ded);
