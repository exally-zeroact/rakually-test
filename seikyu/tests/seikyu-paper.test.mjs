...
T('★★振込先は枠で囲って 口座番号を大きく等幅にする（白黒でも分かる）★★', () => {
  ...
  /* ★2026-09-10 決めが 変わった★（司さん
       「そもそも どこで 改行と やなしに ★字を 少し 小さくしても 1行で 収めれないか？★」
        →「★収まるなら 口座番号は 目立つように★」）
     ＝★勝手に 名義を 次の行へ 送るのを やめた★（2026-08-16 から 私が やっていた事）。
     ★実測★ 本文 7.5pt・口座番号 13pt で 世の中の 長い 例（信用組合＋長い社名）も
       418px＝左の列（455px）に 1行で 収まる。
     ⇒ ここで 見るのは ★1口座が 1行で 出る★事と ★口座番号が 大きい★事。 */
  const long = bankOf('三菱UFJ銀行　丸の内中央支店　当座　1234567　カ）ゼロアクトコーポレーションジャパン');
  ok(long.indexOf('<br>') < 0, '★1口座なのに 割っている★: ' + long);
  ok(/<span class="bank-no">1234567<\/span>/.test(long), '★口座番号が 大きく なっていない★: ' + long);
  /* ★2口座なら 2行★＝別々の 口座は つながない（司さん 2026-09-10「1行で まとめなや」） */
  const futatsu = bankOf('サンプル銀行　サンプル支店　普通　1234567　カ）サンプル'
    + String.fromCharCode(10) + 'テスト銀行　テスト支店　当座　7654321　カ）サンプル');
  eq(futatsu.split('<br>').length - 1, 1, '★2つの 口座が 1行に つながっている★: ' + futatsu);
...
/* ★② 線は「上」に統一★（司さん 2026-08-16「小計だけ下に線」） */
/* ★紙の中の線は1種類★（司さん 2026-08-16「小計の濃い上線はダサい／他と統一」）
...
/* ★A4 1枚に載る数は「測った数」＝勝手に上げたら赤★
   ...
   ★紙に何かを足した日／詰めた日に この数は変わる★
     ...
     → 21/11（振込先の名義を必ず次の行に）→ ★21/10（箱に字の余白を入れた）★→ 18/8
     ★2026-08-31（自社の塊を「ご請求金額」と 下そろえにした日）★
       一度 20/10 に上げたが、上げた理由は ★私が 頭の余白を 詰めたから★で 差し戻された。
       余白を 元に戻したので ★数も 18/8 のまま★（表の始まり 79.9mm＝元と同じ）。
       測り直し（★元と同じ Chromium★）… 18行=余り0px／19行で+1px、控除あり 8行／9行で+14px。
       ★WebKitで測ると 19行に見える★＝★台帳の数は 元と同じ engine で 測る★
     ★2026-09-08 18/8 → 17/7 → ★16/7★★（司さん「他のテンプレも…（項目の合計など）」
       →「は？控除なしのやつって言わんかったか？」）
       ＝表の 一番下に ★「項目の合計」の 1行★を 足した＝明細に 使える 行が 1つ 減る。
       ★測らずに 変えていない★＝減らさずに 出したら
         CI(Linux) 1124px＝A4を 1px 超え／手元(WebKit) 1145px＝22px 超え
         （`seikyu/tests/memo-waku.mjs` が 捕まえた）。1つ 減らして 1123px に 戻った。
     ★同じ日の 2回目 17→16★（司さん「この赤の線にも 控除ありの時のように 分かりやすくやって」）
       ＝控除が 無い 紙にも ★3つ目の 帯（ご請求金額｜金額）★を 出した＝もう 1行 高く なる。
       ★出す前に 測った★＝減らさずに 出すと 1142px（A4を 19px 超え）。
       ★控除ありは 7のまま★＝帯は 前から 在るので 増えない。 */
T('★★既定の行数は「A4 1枚に収まると実測した数」から動かさない★★', () => {
  /* ★2026-09-10 司さん「下が 重たく 感じるから 赤線の ところの 余白を 少なくして
       ★項目の 行数を 2個 減らして★」★＝18→16／控除ありは 7→5。
     ★上限が 減るのでは ない★＝出す 空行が 減るだけ（多い時は 今までどおり 2枚目へ）。 */
  eq(PAPER.PAPER_ROWS, 16, '★控除なしの既定を測らずに変えた★');
  eq(PAPER.PAPER_ROWS_DED, 5, '★控除ありの既定を測らずに変えた★');
...
T('★★締めの筋道で 小計＋消費税−控除 ＝ 請求額 が必ず一致する★★', () => {
...
/* ★2026-09-03（指示役の裁定＝案B）★ 1枚物の紙は ★表の中に 合計行を 出さない★
   （同じ「小計」が 表の最終行と 締めの 2か所に 出ていた／実物45枚では ★0回★の言葉）。
...
T('★★それぞれのブロックの下に そのブロックの合計が出る（給料明細と同じ）★★', () => {
  ...
  eq((flat.match(/小計/g) || []).length, 1, '★1枚物は 締めに 1回だけ（案B）★');
});
...
T('★★宛先の下に住所を刷らない（名前と敬称・担当者だけ）★★', () => {
  ...
  /* ★2026-08-29 直した★（司さん・指示役 ④の残り「敬称の自動判定」）
     ...
     前は ここで「御中が在る事」を 正としていた＝★間違いを 試験で 固定していた★。
     作法は seikyu-doc.js の addresseeOf が 1か所で持つ。 */
  ok(!/御中/.test(h), '★担当者が居るのに 御中を付けている（御中＋様＝二重敬称）★');
  ok(/山田/.test(h), '★担当者まで消している（誰宛かが分からなくなる）★');
  ok(/山田　様/.test(h) || /山田\s*様/.test(h), '★担当者に 様が 付いていない★');
  /* ★担当者が居ない時は 会社行に 御中★（片方だけ見て 直したと言わない） */
...
T('★★途中の紙は目一杯 載せて 紙を増やさない（最後の紙だけ控除と締めの分 少なく）★★', () => {
  ...
  /* ★★分け方の決め＝前の紙から 順に いっぱいまで★★（司さん 2026-09-10
       「★なんで 1ページ目が 少ないやつがあるんど おかしかろが★」）
     ★前は「最後の紙に 枠ぶん 残し、残りを 途中の紙へ 均等に」★＝
       明細17本・最後の枠15本で ★1枚目に 2本しか 載らなかった★（実測）。 */
  const plan = (n, mid, last) => PAPER.planPages ? PAPER.planPages(n, mid, last) : null;
  if (plan) {
    eq(plan(8, 28, 8).length, 1, '8行で2枚になっている');
    const p2 = plan(26, 28, 8);
    eq(p2.reduce((a, x) => a + x, 0), 26, '★分けたら本数が変わった（行が落ちる）★');
    eq(p2[0], 26, '★1枚目に いっぱいまで 入れていない★: ' + JSON.stringify(p2));
    /* ★1枚目に 2本しか 載らない、が 起きない★＝後ろの紙が 前の紙より 多くならない */
    const p17 = plan(17, 28, 15);
    ok(p17.every((x, i) => i === 0 || x <= p17[i - 1]),
      '★後ろの紙の方が 多い（1枚目が スカスカ）★: ' + JSON.stringify(p17));
    const p3 = plan(100, 28, 8);
    eq(p3.reduce((a, x) => a + x, 0), 100, '100行で本数が変わった');
    ok(p3.slice(0, -1).every((x) => x <= 28), '★途中の紙が28行を超えている（切れる）★: ' + JSON.stringify(p3));
    ok(p3[p3.length - 1] <= 8, '★最後の紙が 枠を 超えている★: ' + JSON.stringify(p3));
    ok(p3.every((x, i) => i === 0 || x <= p3[i - 1]),
      '★後ろの紙の方が 多い★: ' + JSON.stringify(p3));
    /* ★空振りしない★＝本当に 前から 詰まっている（全部 同じ数では 見ていない） */
    ok(p3[0] === 28, '★1枚目が いっぱいでない★: ' + JSON.stringify(p3));
  }
});
...
T('★★控除が増えたら1枚に載る明細が減る（枠4件までは変わらない）★★', () => {
  ...
  /* ★数を 焼き付けない★（2026-09-10 控除ありの 既定が 7→5 に なって この式が 破綻した）
     ＝見たいのは ★増えたら 減る／減り方が 大きくなる／0を 下回らない★ の3つ。 */
  ok(M(true, 1, 5) < M(true, 1, 4), '★控除5件で減っていない（超えた分＋保険1行）★');
  ok(M(true, 1, 8) <= M(true, 1, 5), '★控除が 増えたのに もっと 減っていない★');
  ok(M(true, 1, 8) >= 0, '★0を 下回った★');
  eq(M(false, 1, 8), PAPER.PAPER_ROWS, '控除を出さない紙で減らしている');
  ok(M(true, 1, 99) >= 1, '★控除が極端に多い時に 0行や負の数にしている★');
  /* ★数を 焼き付けない★（既定が 変わると 破綻する）＝★件数が 効いているか★を 見る */
  ok(PAPER.frameRowsOf({}, { deduct: 1, dedLines: 8 })
    < PAPER.frameRowsOf({}, { deduct: 1, dedLines: 4 }),
        ...
        for (const taxMode of ['exclusive', 'inclusive']) {
          ...
          b.html.split('class="sheet"').slice(1).forEach((s, i) => {
            ...
            if (hasFoot) {
              ...
              const wantA = (taxMode === 'inclusive') ? S2['合計'] : S2['小計'];
          ...
          if (taxMode === 'inclusive') {
            ...
            if (!Object.keys(S).some((k) => /小計（税抜）/.test(k))) ngLabel.push(tag + ' 締めに（税抜）が無い');
...
T('★★表の中は「このページの小計」・締めは「全ページの」＋締めは何枚でも同じ行数★★', () => {
  ...
  /* ── 1枚物 ── 表の中は「小計」／締めに「全ページの」は要らない */
  const one = mk(5);
  eq(one.pages, 1, '5行で複数ページになっている');
  ok(/小計/.test(one.html), '1枚物に「小計」が無い');
  ...
  sheetsOf(many.html).forEach((s, i) => {
    ...
    /* ★2026-09-03★ 足元の 言葉を「明細の合計」→★小計★に した為、ここを そのまま 置き換えると
       上の行（このページの小計）と ぶつかる。★見たい 事＝「表の 足元に 出る 小計は ★このページの分だけ★」★
       ⇒ ★数で 見る★（小計の 数 ＝ このページの小計の 数）。 */
    const zenbu = (foot.match(/小計/g) || []).length;
    const page = (foot.match(/このページの小計/g) || []).length;
    ok(zenbu === page, '★' + (i + 1) + '枚目の表に 全体の合計を書いている★（小計 ' + zenbu + '／このページの小計 ' + page + '）');
  });
  ok(/全ページの\s*小計/.test(many.html.replace(/<[^>]+>/g, '')),
    '★締めに「全ページの 小計」が無い（表の数と何が違うのか分からない）★');
  ...
  eq(tally, S['全ページの 小計'],
    '★各ページの小計を足しても 締めの「全ページの 小計」にならない★');
...
T('★★頭の並びが どの紙も同じ（挨拶→金額→ページ番号→明細）★★', () => {
  ...
  sheets.forEach((p2, i) => {
    const o = order(p2);
    const last = (i === sheets.length - 1);
    /* ★まとめの紙★＝最後の紙に 明細が 1本も 来なかった時（2026-09-10 の 分け方から ふつうに 起きる）
       ＝表そのものを 出さない（空の表と「このページの小計 0」を 出さない）ので、
         「ページ番号が 明細の すぐ上」は そもそも 比べる 相手が 無い。 */
    const matome = (o.明細 < 0);
    ok(!matome || last, '★途中の紙に 明細の表が 無い★');
    if (!matome) ok(o.ページ >= 0 && o.ページ < o.明細, '★' + (i + 1) + '枚目：ページ番号が明細のすぐ上に無い★');
    else ok(o.ページ >= 0, '★まとめの紙に ページ番号が 無い★');
    if (i === 0) {
    ...
    if (last) {
      ...
      if (!matome) ok(o.ページ < o.明細, '最後の紙：ページ番号が明細より下');
    }
  });
});
...
T('★★「ご請求金額」はいつも紙の頭・複数ページなら最後の1枚だけ★★', () => {
  ...
  sheets.forEach((p2, i) => {
    ...
    if (last) {
      ...
      ok(iG >= 0, '★最後の紙に 金額が 無い★');
      /* ★まとめの紙（明細の表が 無い）★でも 場所の決めは 同じ＝締めより 上・頭に 在る */
      ok(iT < 0 || iG < iT, '★最後の紙で金額が明細より下に出ている（場所が違う）★');
