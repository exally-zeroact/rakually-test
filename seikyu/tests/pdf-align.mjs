...
T('★測れている（PDFが出来て 字も 置かれている）', () => {
  /* ★★大きさの 幅（下限・上限）の 出どころ＝実測（2026-09-06・本物のブラウザで 作って 測った）★★
       ★字が 1つも 入らない紙（.notdef だけ）★ … ★252,390 B★
       ★今の 正しい紙★ … ★93,102 〜 102,036 B★（std1 3行／std1 20行／elegant 20行＋備考／koujo 20行）
       ★丸ごと 同梱（直す前の 姿）★ … ★3,093,485 B★

     ★★分かった 事＝「大きさ」では ★字が 入っているか★は 守れない★★
       ＝字が 入らない紙(252,390B)の 方が 正しい紙(93〜102KB)より ★大きい★。
         （対応表(cmap)が 残る為）。★前の コメントで そう 書いたのは 私の 誤りです。★
       ⇒★字が 入っているかは 下の ★置いた字の数（placed）★が 見る★＝そちらが 本体。

     ★この 幅が 守る 物（1つずつ）★
       ★★下限は 外しました★★（2026-09-06・わざと 壊して 確かめた）
         ★30,000B を 割る 壊し方が ★1つも 見つからなかった★★
           ・明細0行・自社も相手も 短い 紙 … ★84,742 B★（割らない）
           ・字体を 1バイトも 埋めない … ★紙が 出ない（例外）★＝大きさでは 捕まらない
           ・字体の 埋め込みを 飛ばす … ★3,087,424 B★＝★上限が 捕まえる★
         ⇒★「在るのに 何も 守っていない 線」＝いつも真＝死んだ 線★なので 外した。
         ⇒★紙の 中身が 落ちたかは ★placed（置いた字の数）★が 見る★＝そちらが 本体。
       ★上限 500,000B★ … ★字体を 丸ごと 同梱する 姿に 戻ったら 赤★。
         ＝★測って 引いた★＝正しい紙の 最大 102,036B と 丸ごと 3,093,485B の ★間★。
     ★前は「10万Bより 大きければ 本物」★＝★丸ごと 同梱していた 時代の 物差し★で、
       軽くしたら ★正しい紙が 下限を 割って 赤に なった★（見張りが 昔の 姿を 守っていた）。 */
  ok(r.bytes < 500000, '★PDFが 大きすぎる（' + r.bytes + 'B）＝字体を 丸ごと 埋める姿に 戻った疑い★');
...
T('★⑤ 様式ぜんぶ・控除あり・複数ページ・見積書でも PDFが 出る', () => {
  ...
  results.forEach((x) => {
    ok(x.size < 500000, '★' + x.name + ' が 大きすぎる（' + x.size + 'B）＝丸ごと 埋めている疑い★');
...
T('★⑦ 角印が PDFにも 入る（絵を 落とさない）', () => {
  ...
  ok(withSeal.size < 900000, '★PDFが 大きすぎる（' + withSeal.size + 'B）＝印の 絵を 入れても この中★');
