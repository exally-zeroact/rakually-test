...
const NOT_WIRED = {
  ...
  /* ★2026-08-30 借りたPDFの道具★（司さん「成功してるアプリを真似て同じ形式でやれや」）
     ★まだ 画面から 呼んでいない★＝ボタンを 出していない。
     ★理由★＝借りた道具は ★代行の商売の決まり★を 持っている（実測）:
       ・請求日を ★月の翌月1日★で 自分で作る（うちの請求日を 渡せない）
       ・外税（税抜で入れる相手）だと 見出し「御請求金額（税込）」が ★税抜の額★になる
     ⇒ ★内税の1通なら 今すぐ 正しく出る★（実測で 小計/税/合計とも 一致）が、
       ★外税で 出すと 紙が 嘘をつく★ので ★ボタンは 出さない★
       （[[feedback_dont_show_unfinished_buttons]]）。 */
  'seikyu/lib/invoice-pdf.js': {
    reason: '代行請求書アプリから1文字も変えずに借りたPDFの道具。'
      + '内税の1通は正しく出る事を実測済み。外税と請求日は 道具の中を直さないと 直らない。',
    wireWhen: '司さんが「内税だけ先に出す」か「元（代行）を直す」を決めた時。',
    owner: '司さん（どちらで進めるかの一言）',
  },
  'seikyu/lib/seikyu-pdf-bridge.js': {
    reason: '上の道具へ Rakunally の中身を 渡す役。道具と一緒に 使う物なので 今は 呼んでいない。',
    wireWhen: 'invoice-pdf.js を画面に繋ぐ時（同時に繋ぐ）。',
    owner: '司さん（同上）',
  },
};
