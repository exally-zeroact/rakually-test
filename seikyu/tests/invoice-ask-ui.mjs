 ...
 *   ⑦ 発行済みでも 聞く（2026-09-09 いつでも直せるように なった）
 *
...
T('★⑦ 発行済みでも 聞く（いつでも 直せるように なったので）', () => {
  /* ★2026-09-09 決めが 変わった★（司さん「代行請求書のように いつでも編集できるように」）
     ＝発行済みも 直せる。★直せる物に 聞かないのは おかしい★ので ここも 出す。
     前は「触れない1通には 聞かない」だったが、触れない1通そのものが 無くなった。 */
  reset({ status: 'issued' });
  ok(shown(), '★直せるのに 聞いていない★');
  console.log('     発行済み → 出る（直せるので）');
});
