...
T('2-a. ★タブの順と動詞を給与にそろえた（設定→入力→一覧・「作る」ではなく「入力」）', () => {
  ...
  /* ★★2026-09-10 「請求/集計」を 外して 3つに 戻した★★
     司さん「一覧と集計の差は なんなん 同じことが できるなら いらんことないか？」
     ★数えて 確かめた★＝一覧の 一部だった
       ・集計の 計算は 同じ REP.summarize を 2か所で 呼んでいた
       ・絞り込みの 欄も 2組／出す ボタンは 2組で 中身が ちがった（納品書が 出せない）
       ・分ける 理由（選ぶだけで 紙が 出る）は 確認を かぶせに して 無くなった
     ★集計は 消していない★＝一覧の 頭に 出しっぱなしに した（前は 引き出し2枚の 下）。
     ★給与と 同じ 3つ★（設定・入力・一覧）。 */
  eq(tabs.join(' '), 'scr-set:設定 scr-edit:入力 scr-list:一覧',
    'タブの順か言葉が違う');
  /* ★集計が 消えていない★＝一覧の 頭で すぐ 見える（引き出しの 中に 戻さない） */
  ok(doc.getElementById('rep-sum'), '★集計が 無くなっている★');
  ok(doc.getElementById('rep-month'), '★集計する月を 選べない★');
  ok(!doc.getElementById('rep-box'), '★集計が また 引き出しの 中に 入っている★');
...
await TA('14-a. ★同じ数字を2回 言わない（税率が1つなら「◯%対象」を出さない）', async () => {
  ...
  /* ★2026-09-03 言葉を 決め直した（指示役の裁定＝C）★
     ★実物45枚（16社）で「明細の合計」は 0回★／足元は ★小計 → 消費税 → 合計★。
     ★この試験の 見たい事＝「画面と 紙で 同じ 言葉」★は そのまま。言葉だけ 入れ替えた。 */
  ok(/小計/.test(tot) && /消費税/.test(tot) && /合計/.test(tot), '小計・消費税・合計が出ていない');
  ok(!/明細の合計/.test(tot), '★画面だけ 古い言葉「明細の合計」のまま（紙は「小計」）★: ' + tot.replace(/\s+/g, ' '));
