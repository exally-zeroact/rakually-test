 ...
 *
 * ★大きさ★（★実寸ではなく「紙に出る大きさ」★）
 *   実物の判子は … 会社の角印 21mm角（18〜24mmが多い）／個人の印 10.5〜18mm
 *   ★だが 紙には 実寸では押さない★
 *     ＝代行請求の紙（invoice-pdf.js:762）は ★設定mm × 0.8★ で押している
 *       （既定20mm → 紙の上では 16mm）。★うちも その基準に合わせる★。
 *   ⇒ 角印 21mm×0.8 ≒ ★17mm★ ／ 丸い印 15mm×0.8 ≒ ★12mm★ を ★当てて見せる★
 *   ★違えば その場で直せる★（うちの決まり「聞いてあげる。埋めさせない」）
 *   ★実測（司さんの実物の角印・2026-08-30）★
 *     21mmで押すと 自社の63文字のうち ★28文字★が 印にかかった（社名・住所・電話）
 *     17mmなら ★14文字★（社名の末尾と 住所の末尾だけ）＝代行の紙と 同じ見え方
 *
 * ★分からない時は 分からないと言う★
    ...
    if (c <= MARU_MAX) {
      return { shape: 'maru', mm: MM_MARU, sure: true,
        why: '四隅が 空いているので、丸い印（個人の苗字の印）と見ました。'
          + '個人の印の実物は 12〜18mm で、紙には その8割で押すので ' + MM_MARU + 'mm にしました。' };
    }
    return { shape: 'unknown', mm: MM_KAKU, sure: false,
