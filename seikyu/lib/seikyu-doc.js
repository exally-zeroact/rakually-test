...
(function (root, factory) {
  ...
  /* ★既定は 20mm★（司さん 2026-09-10「デフォが17mmになってるのを ★20mmにしといて★」）
     ★前は 17mm★＝実寸21mmの角印を 紙には その8割で押す、という 決め方だった
       （司さん 2026-08-30 実物の角印で 測り直した数）。司さんの 決めで 20mm に した。
     ★3か所 同じ数に する★＝seikyu-doc（決まり）／seikyu-paper.sealMm（紙）／
       seikyu-seal.MM_KAKU（形から 当てる 時）。
       （2026-08-31 実測で 食い違っていた＝画面は「既定21mm」と言い、紙は 17mmで押していた） */
  var SEAL_DEFAULT_MM = 20;
  var SEAL_MIN_MM = 10;
  var SEAL_MAX_MM = 40;
  ...
  function validateInvoice(o) {
    ...
    /* ★お振込先が 空★（2026-09-09 司さん
       「振込先は 設定で 入れたら 出るんか？ 今は 入れんかったら 出てないけん それでええんか？」）
       ★実測（2026-09-09）★ 入れれば「お振込先」の 箱が 紙に 出る／
         空なら ★箱ごと 出ない★。しかも 発行の 検査に 振込先は 1つも 無く、
         ★何も 言わずに 振込先の 無い 請求書が 出ていた★。
       ⇒ ★止めない。黙らない★（うちの決まり）
         ・止めない … 現金・手形・相殺など ★振込先が 要らない 請求も 在る★
         ・黙らない … 振込先が 無い 紙は ★客が どこへ 払えばよいか 分からない★
       ★領収書・納品書は 支払いの 依頼では ないので 見ない★（この検査は 請求書・見積）。 */
    if (!String(od.bank || '').trim()) {
      warnings.push('お振込先が空です（設定 ▸ 自社の情報 で入れられます）。'
        + 'このままでも出せますが、紙にお振込先が出ません');
    }
