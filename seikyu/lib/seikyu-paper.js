    ...
    var sheets = pages.map(function (pageLines, idx) {
      ...
      var body = ''
        ...
        + (function () {
          ...
          /* ★控除を 出す紙は「項目の合計」と 呼ぶ★（2026-09-08 司さん「赤丸の所に項目の合計は？」）
             ＝控除を 出す紙は ★塊が 3つ★（項目／控除明細／請求額の計算）。
               下の 締めの「小計」と ★同じ数★でも 役目が 違う＝
               ここは ★①の 塊の 合計★（②の「中計」と 同じ役目）。 */
          if (showDeduct && last && !multi) footLabel = textOf(TH.itemSum) || '項目の合計';
          /* ★税込で打つ紙は「（税込）」と書く★
          ...
          /* ★控除を 出す紙では 表の 合計行を 消さない★（2026-09-08）
             1枚物で 消していたのは ★塊が 1つの 紙で 同じ数が 2回 出ていた★から。
             控除を 出す紙は ★塊が 3つ★＝②に 中計が 在って ①に 何も 無いと
             ★どこまでが 項目の 話か 分からない★（司さんの 赤丸）。 */
          var dropFoot = (last && !multi && !showDeduct);
