 ...
 *   今 … ★枠・RA・字 #52B788（ミント）／チェック #F5811F（橙）★
 *   ★選び方（司さん 2026-08-29「薄緑に合うアクセントを何種類か見せて」→ 8色出して 橙を選択）★
 *     ここまでに 2回 外している:
 *       #2E7D54（浮き2.03・濃い）… 「チェックにしか目がいかん」＝★一番 重い物になる★
 *       #C8ECD8（浮き1.94・薄い）… ★32pxで ほぼ 消える★（自分で 切り出して 見た）
 *       #3D9E72（浮き1.34）　　 … 重くはないが ★アクセントに ならない★（枠と一体）
 *     ★#F5811F（浮き1.06）★   … ★重さは 枠と ほぼ同じ★のに ★色が違うので 一目で分かる★。
 *   ＝★「明るい/濃い」ではなく『重さは そのまま・色だけ 変える』★が 答えだった。
 *     ⇒ 下の自己確認で ★浮き 1.5 未満★を 決まりにした（重さで 目を奪わせない）。
...
/* ★色（司さん 2026-08-29「全部ミントでチェックだけ変えろや」）★
     ...
     ⇒ 枠・RA・字 → ミント ／ チェック → ★橙 #F5811F★（司さんが 8色から 選んだ）。
     ...
     ★2色とも 元から在るブランドの色★
       ...
       #F5811F … ★ロゴのチェックだけ★（2026-08-29 に 足した。★画面には 広げない★）
     ＝★新しい色を 作っていない★。 */
export const WORD_INK = [82, 183, 136];    // #52B788 ミント … 枠・RA・字（＝画面の頭と同じ色）
export const CHECK_INK = [245, 129, 31];   // #F5811F 橙 … チェックだけ（司さん 2026-08-29 が選んだ）
/** 元絵の色 → 新しい色。★この2本だけ★（他の色が混ざっていたら 赤にする） */
export const SWAP = [
  { from: [46, 125, 84], to: WORD_INK, why: '枠・RA（元は濃い）→ ミント' },
  { from: [82, 183, 136], to: CHECK_INK, why: 'チェック（元はミント）→ 橙' },
];
...
export const BRAND_COLORS = {
  ...
  '#F5811F': '★ロゴのチェックだけ★（2026-08-29 司さんが選んだ・画面には広げない）',
  '#F0FAF4': '地の色',
};
