-- ════════════════════════════════════════════════════════════════════════
-- Rakunally 共通の棚 — 入口(共有データ)・給与・請求書が一緒に使う3つ
--   ★2026-08-17 に supabase/schema-exally.sql から改名した★（中の DDL は1行も変えていない）
--   ・名前が「exally」だと、Rakunally の棚なのに Exally の物に見える（司さん「Rakually は別アプリ」）
--   ・★元は頭に「倉庫は…(本番のプロジェクトID)」と書いてあった＝貼る先を書いた紙は
--     「間違って本番の部屋に当てる」事故の元★（指示役 2026-08-18）。だから ★倉庫のIDは書かない★。
--   ★どこに当てるか★＝いま自分が向いている倉庫（js/supa-config.js の url が唯一の正）。
--     テスト線は DB-test。★本番へ当てるのは指示役の号令で1回だけ★。当てる前に
--     node seikyu/tests/sql-guard.test.mjs（消す系・他の棚・security_invoker抜けを止める門番）を通す。
--
...
--                    ※ pay_companies は 給与が data を丸ごと上書きするため共有不可
--                       (js/store.js:73,95 + js/app.js:3036 で実測)→ 別テーブルにする
--     pay_partners ... 取引先マスタ(請求/見積の源)
--     pay_ledger   ... 日次/期間台帳(人×日付×{売上/時間/金額}) ＝ 二度手間解消の実体
--
