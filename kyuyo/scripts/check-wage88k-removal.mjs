 ...
 *   ★期限 2027-10-01★ までに必ず1回、人が見て切替を判断する。
 *
 *   ★★日付を 引き直した 訳（2026-09-16・実測してから 決めた）★★
 *     ★前の 期限 2026-09-15 は 過ぎた★。★壊れては いない＝「もう一度 人が 見る 日」が 来ただけ★。
 *     ★今日 法令を 引いた 結果★（この紙を そのまま 走らせた・終わり値 0）:
 *       厚年 … 2026-05-25／10-01／12-01／2027-09-01／10-01／2028-04-01／2028-06-13
 *       健保 … 2026-08-01／10-01／2027-04-01／05-20／06-04／2027-10-01
 *       ★上の 全部の 施行版に「八万八千円」が 1件ずつ 残って いる＝撤廃は まだ 入って いない★
 *     ★なぜ 2027-10-01 か★
 *       ・★★両方の 法で 確かめた 先の うち『手前で 終わる 方』の 最後の 日★★＝
 *         今日なら 厚年 2028-06-13／健保 ★2027-10-01★ ⇒ ★手前で 終わるのは 健保★ ⇒ 2027-10-01。
 *         ★そこまでは『撤廃が 入って いない』と ★両方の 法で★ 実物で 確かめ済み★。
 *         ⇒ ★確かめた 先より 手前に 期限を 置いても、人は 同じ 答えを 見るだけ★（空振り）。
 *         ★★『一番 先（＝2028-06-13）』を 採っては いけない★★＝
 *           ★2027-10-01 より 先の 健保の 版を 1つも 見て いない★／
 *           ★条文は 両方の 法から 消えないと 撤廃では ない★
 *           ⇒ 2028-06 まで 延ばすと ★健保だけ 先に 消えた 時に 気づかない 窓★が 8か月 空く。
 *       ・★近すぎる 期限は 害に なる★＝過ぎるたび「また 見る」を 呼ぶが 中身は 変わらない
 *         ⇒ ★人が 期限に 慣れて 読まなく なる★（今日の『赤が 常態に なると 本物の 赤が 埋もれる』と 同じ 形）
 *       ・★遠すぎても 危なく ない 訳★＝★週1が 毎週 見て いて、消えた 瞬間に 終わり値 3 で 赤★
 *         ＝★人の 期限は あくまで『週1が 生きて いるか』の 控え★（主役では ない）
 *     ★指示役1 の 案は 2026-12-15 でした★。私が 変えた のは 上の 2つ目の 訳
 *     （★確かめ済みの 先より 手前に 置くと 空振りを 増やす★）。★人の 口では なく 実測で 決めた★。
 *     ★★次に 引き直す 時（ここを 読み違えると 穴が 空きます）★★
 *       ①この紙を 走らせる ②★厚年の 最後の 日★と ★健保の 最後の 日★を 両方 見る
 *       ③★小さい方（手前で 終わる 方）★を そのまま 使う ＝ min(厚年, 健保)
 *       ★『一番 先の 日』では ありません★（2026-09-16 に 私が そう 書いて 指示役1 に 直されました）
 *
...
if (needSwitch || odd) {
  console.log('\n★人がやること: kyuyo/lib/shaho-kanyu.js の WAGE_88K_REMOVED_YM を直し、'
    + 'tests/shaho-kanyu.test.js の境界（前月/当月/翌月）を実物で測り直す。期限 2027-10-01。');
  process.exit(3);
}
console.log('\n今のままでよい（法令と lib が一致）。次に見る日: 2027-10-01 まで');
