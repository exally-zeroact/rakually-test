...
(function(){
  ...
  function santeiCsvBox(rows, year){
    ...
    var tomeru=[];
    if(!c.seiriKigou) tomeru.push('事業所整理記号が まだです');
    if(!c.jigyoshoNo) tomeru.push('事業所番号が まだです');
    if(!(window.TodokedeCsv.KEN_CODE[c.pref])) tomeru.push('会社の 都道府県が まだです（設定 ▸ 会社情報）');
