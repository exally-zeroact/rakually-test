...
(function(){
  ...
  // ★仮計算の札★＝文は lib 1か所（画面も 紙も 同じ 言い方にする・2026-09-03 指示役の裁定3）
  function kariNote(e){ return PW().kariKeisanNote(e, state.month); }
  // ★警告の 箱だけ 描き直す★（丸ごと renderEmpMaster しない＝札が 閉じる・場所が 飛ぶ）
  function refreshEmpWarn(){ var el=$('#emp-warn'); if(el) el.innerHTML=prefMissingWarn()+prefTokyoNote(); }
  // 画面用＝対象月に 居る人の うち 県が 空の人が 1人でも 居れば 1行 出す（0人なら 空）
  function kariWarn(){
    var who=activeEmployees().filter(function(e){ return !!kariNote(e); });
    if(!who.length) return '';
    var nm=who.map(function(e){ return e.name||'（無名）'; });
    return '<div class="cr-warn" style="margin:0 0 10px">※ <b>仮の計算です</b>（'
      +(nm.length<=2?nm.join('・'):(nm[0]+'ほか'+(nm.length-1)+'名'))+'）。'+esc(kariNote(who[0]))
      +'。設定 ▸ 従業員マスタ で県を選ぶと 正しい額になります。</div>';
  }
  ...
  function defEmp(name){
    return { id:uid(), name:name||'従業員 1', no:'', birthYmd:'1980-05-15', dept:'', role:'', employmentType:'employee', houshuKubun:'none',
      payType:'月給', base:'', hourly:'', kana:'', seibetsu:'', jushoKana:'', kisoNenkin:'', commissionAmt:'', hourlyGuarantee:'', salesAmt:'', pieceCount:'', payRule:null, fuyou:'1', nenshoFuyo:'', pref:'', commute:'', commuteType:'public', commuteKm:'', residentTax:'', residentTaxMode:'monthly', residentTaxAnnual:'', residentTaxIkkatsu:false, juminCollect:'special', bank:'',
      ...
      /* ★家族（被扶養者）★（2026-09-08 司さん「出来てないものは全部やれや」）
         被扶養者(異動)届（様式2202700）に 要る＝★人数では 出せない★（氏名・生年月日・続柄…が 要る）。
         ★空の 一覧で 置く★＝入れていない人には 何も 増えない（今までの 従業員も そのまま 動く）。
         中身の 形は lib(todokede-csv.js) の sonotaBlock が 受け取る 物と 同じ 名前に そろえる。 */
      kazoku:[] };
  }
  ...
  /* ═══════════════════════════════════════════════════════════════════
     ...
     ・★聞かない物★＝割増率(法定)／労災率(全額 事業主負担＝明細の控除に出ない／★2026-09-04 に 法令の 別表第１から 表を 持った＝業種を 選ぶ★)／
  ...
  var CO_FIXED=['割増の率（残業・休日・深夜・月60時間超）'];
  ...
  function empCardBody(e,i){
    ...
    /* ★家族（被扶養者）★（2026-09-08 司さん「出来てないものは全部やれや」）
       ★なぜ 要るか★＝被扶養者(異動)届（様式2202700）は ★人数では 出せない★。
       氏名（カナ・漢字）・生年月日・性別・続柄・同居別居・住所・収入 が 1人ずつ 要る
       （年金機構の 仕様書 令和8年3月 第16.2版 表４．１０．１－１）。
       ★聞くのは 届出に 要る物だけ★＝それ以上の 個人情報は 預からない。
       ★個人番号（マイナンバー）は 聞かない★＝持たない物は 漏れない（届出は 住所で 出す）。
       名前は lib(todokede-csv.js) の sonotaBlock が 受け取る 物と そろえてある。 */
    /* ★続柄コードは 原文のとおり★（仕様書 項番76）
       ★前に 自分で 作った表（夫01 妻02 子03…）は 全部 嘘だった★
       特に 03 は「子」ではなく「父母」＝子を 親として 届け出す 所だった。
       ★配偶者は この表に 居ない★＝原文では 別の 場所（項番22〜69）に 書く。
       だから 画面では 'haigusha' という 別の 選択肢で 受けて、出す時に 振り分ける。 */
    var ZOKU = [['', '（選んでください）'], ['haigusha', '配偶者（夫・妻）'],
      /* ★言い方だけ 変えた（2026-09-14 司さん）★＝★コードは 1つも 動かしていない★。
         原文（項番76）は 01「実子・養子」／02「１以外の子」。
         客の 画面で 02 が ★何の 話か 分からない★ので ★頭に「子」を 付ける★だけに する。
         ★原文に 無い 例（連れ子 等）は 書かない★＝原文が そう 言っていないので 当て推量に なる。 */
      ['01', '子（実子・養子）'], ['02', '子（それ以外）'], ['03', '父母・養父母'], ['04', '義父母'],
      ['05', '弟妹'], ['06', '兄姉'], ['07', '祖父母'], ['08', '曾祖父母'], ['09', '孫'], ['10', 'その他']];
    /* ★職業と 理由は 配偶者と それ以外で 中身が 違う★（項番48/46 と 項番92/95）
       ★1つの 表を 使い回すと 年金機構に 弾かれる★ */
    var SHOKU_SONO = [['', '（選んでください）'], ['1', '無職'], ['2', 'パート'], ['3', '年金受給者'],
      ['4', '小中学生以下'], ['5', '高校・大学生'], ['6', 'その他']];   /* ★390pxで 切れていた★（絵を 見て 短くした） */
    ...
    var gKazoku = (function () {
      ...
      var h = '<div class="ri-note" style="margin:0 2px 8px">健康保険の <b>被扶養者(異動)届</b>（様式2202700）に使います。'
        + '<b>入れた人だけ</b>が届出に出ます。<b>マイナンバーは預かりません</b>（届出は住所で出します）。<br>'
        + '<b>郵便番号と住所は 同居でも要ります</b>（年金機構の決まり：個人番号の有無・同居別居に関わらず省略不可）。</div>';
      h += list.map(function (k, ki) {
        var pre = 'data-kz="' + i + ':' + ki + ':';
        var isHai = (String(k.zokugara || '') === 'haigusha');
        var shokuT = isHai ? SHOKU_HAI : SHOKU_SONO;
        var riyuT = isHai ? RIYU_HAI : RIYU_SONO;
        var erabi = function (tbl, ima) {
          return tbl.map(function (z) {
            return '<option value="' + z[0] + '"' + (String(ima || '') === z[0] ? ' selected' : '') + '>' + esc(z[1]) + '</option>';
          }).join('');
        };
        return '<div class="card" style="padding:10px;margin:0 0 8px">'
          ...
          + ['', 'male', 'female'].map(function (v) {
            return '<option value="' + v + '"' + (String(k.seibetsu || '') === v ? ' selected' : '') + '>'
              + (v === 'male' ? '男' : v === 'female' ? '女' : '（選んでください）') + '</option>'; }).join('')
          + '</select></div></div>'
    ...
    var gTeate=''
      ...
      /* ★資格取得届（電子申請）に 要る 2つ★（2026-09-05）
         ★このアプリは マイナンバーを お預かりしない★ので、年金機構の 相関チェックで
         「個人番号が 省略されている場合 → 郵便番号・住所（カナ）は 入力されていること」に なる。
         （出どころ zidoucheck.files/csv200_202412.pdf 項番29・30・31）
         種別（性別）も 取得区分'1'なら 必須（項番11）。 */
      +'<div class="frow2"><div class="frow"><div class="flabel">性別<span class="hint2">資格取得届の「種別」に使います</span></div>'
        +'<select class="finput m-f" data-f="seibetsu"><option value=""'+(!e.seibetsu?' selected':'')+'>選んでください</option>'
        +'<option value="male"'+(e.seibetsu==='male'?' selected':'')+'>男性</option>'
        +'<option value="female"'+(e.seibetsu==='female'?' selected':'')+'>女性</option>'
        +'<option value="kounai"'+(e.seibetsu==='kounai'?' selected':'')+'>坑内員</option></select></div>'
        +'<div class="frow"><div class="flabel">住所（カナ）<span class="hint2">半角カナ・資格取得届に使います</span></div><input class="finput m-f" data-f="jushoKana" value="'+attr(e.jushoKana)+'" placeholder="ﾄｳｷｮｳﾄｼﾌﾞﾔｸ 1-2-3"></div></div>'
      /* ★基礎年金番号★（司さん 2026-09-05「競合がやりよることはやれ」）
         ★なぜ 要るか★＝退社の 届出（資格喪失届 2201700）は 原文 項番12 で
           「★個人番号に 入力がない場合 入力されていること★」＝
           ★マイナンバーか 基礎年金番号の どちらかが 必須★。
           うちは ★マイナンバーを 預からない★ので ★こちらを お預かりする★。
         ★競合も 同じ形★＝マネーフォワードは 従業員ごとに どちらかを 選ばせている
    ...
    var detail=(isContractor
        ...
        : subsec('zaiseki','在籍・勤務',gZaiseki)+subsec('zei','税・住民税',gZei)+subsec('shaho','社会保険',gShaho)+subsec('kazoku','家族（被扶養者）',gKazoku)+subsec('teate','通勤・手当・振込・控除',gTeate))
  ...
  function renderEmpMaster(){
    ...
    /* ★警告は 名前の付いた 箱に 入れる★（2026-09-03 穴②）
       前は 裸で 置いていた為、県を 直しても ★画面を 描き直すまで 警告が 出たまま★だった
       （実測＝空にした 直後は 0個・描き直すと 1個・直しても 1個のまま・もう一度 描き直して 0個）。
       ⇒★この箱だけ 描き直す★＝★開いていた 札を 閉じない・入力中の 場所を 飛ばさない★。 */
    var html='<div id="emp-warn">'+prefMissingWarn()+prefTokyoNote()+'</div>';
  ...
  function rowsHTML(g,arr){
    return arr.map(function(it,ri){
      ...
      if(g==='shikyu'){
        ...
        /* ★現物の印★（2026-09-03 指示役の裁定＝D-3-1）＝算定基礎届の ⑫「現物によるものの額」用。
  ...
  function importKintaiCsv(text){
    ...
    var extra=(r.skipped&&r.skipped.length?'\n読まなかった列: '+r.skipped.join('・'):'')
      ...
      /* ★直せない欠けは 客に伝える（黙って混ぜない）★（指示役 2026-08-24）
         見出しが「深夜」だけの時、それが ★休日の深夜を含むかは 言葉からは分からない★。
         含んでいると 休日深夜(60%)が 深夜(25%)として入る＝★払い足りない★。
         ★中身は読めない＝こちらでは直せない★ので、★その会社に分けてもらう★ために1行 出す。 */
      +((r.map&&r.map.night!=null&&r.map.holidayNight==null)
        ? '\n※「深夜」に 休日の深夜が入っているかは、この表からは分かりません。'
          +'休日に深夜がある会社は「休日深夜」を分けた列で出してください'
          +'（分けないと 休日深夜が 深夜として計算されます）。' : '');
  ...
  function renderBonus(){
    ...
    var head='<div class="card" style="padding:12px;margin-bottom:10px">'
      ...
      /* ★置き字を 日付の 形に★（2026-09-03）＝銀行に 送るには 年-月-日 が 要る。
         ★入っている 自由文は 消さない★（読めた時だけ 振込に 使う・紙は 今までどおり）。 */
      +'<label style="font-size:12px;color:#2E7D54;font-weight:700">支給日 <input class="finput finput-sm" data-bn="payDay" value="'+attr(b.payDay)+'" placeholder="例 2026-12-10" style="width:130px"></label>'
      +'</div>'
    ...
    var cards=state.employees.filter(function(e){return isActiveInMonth(e,ym);}).map(function(e){
      ...
      return '<div class="acc icard'+(num(en.amount)>0?' open':'')+'">'
        +'<div class="ic-top"><span class="acc-nm">'+esc(e.name)+'</span><span class="acc-net"'
          +(c.taxMitei?' style="color:#C0392B;font-size:12px"':'')+'>'
          +(c.taxMitei?'源泉が 未定':yen(c.net))+'</span></div>'
    ...
    var _machiNin = state.employees.filter(function(e){ return isActiveInMonth(e,ym); })
      .filter(function(e){ var cc=computeBonus(e); return cc.taxMitei && (bonusEntry(e).amount>0); });
    ...
    var footer = anyBonus
      ? '<div class="card" style="padding:12px;margin-top:6px"><button class="btn-primary" data-confirm-bonus style="width:100%"'
        +(_machi?' disabled':'')+'>'
        +(_machi
          ? 'この賞与を確定（'+_machi+'名の 源泉が 決まっていません）'
          : 'この賞与を確定（年調・台帳に反映）')+'</button>'
        +(_machi
          ? '<div class="cr-warn" style="margin:8px 0 0">⚠ <b>'+_machi+'名</b>は 前月の給与（社保を引いた後）が 無いので'
            +' <b>源泉所得税が 決まりません</b>（賞与の 源泉は 前月給与で 率が 決まります／特例は 月額表）。'
            +'このまま 確定すると <b>源泉0の 賞与</b>が 年末調整と 賃金台帳に 入ります。'
            +'<br>各人の 赤い箱に 前月の額を 入れるか、前月を 計算して 確定してください。'
            +'（特例で「月額表で要計算」と 出ている人は その額を 確かめてから）</div>'
          : '')
  ...
  function renderListView(){
    ...
    host.innerHTML=statutoryStaleWarn()+kariWarn()+state.employees.filter(function(e){return isActiveInMonth(e,state.month);}).map(function(e){
  ...
  function renderSumView(){
    ...
    $('#view-sum').innerHTML=kariWarn()+warnCard+'<div class="card"><div class="card-h">月次集計（'+mlabel+'）</div>'
  ...
  function renderChinginDaicho(sub){ var host=$('#view-cho'); var year=parseInt(String(state.month||'').slice(0,4),10)||2026;
    ...
    Store.getPayslipsByYm(year+'-01', year+'-12').then(function(recs){ recs=confirmedRecs(recs).filter(function(r){return r.data.kind!=='bonus';}); host.innerHTML=sub+kariWarn()+chinginDaichoHTML(CD().buildLedger(recs, year, state.employees), year); }) // 賃金台帳(月次)は賞与除外
  ...
  /* ★様式の 欄で 出す★（2026-09-03 指示役の裁定＝D-3-1）
     ...
     欄は ①〜⑱。★うちの 列名を 様式の 言い方に 寄せる★＝紙と 並べた時に 迷わない。
     ★⑪通貨／⑫現物／⑬合計★は 月ごとに 3つ 並べる（様式も 月ごとに この3つ）。
     ★⑰個人番号は 持たない（司さん案件・裁定＝甲）★＝★列は 出すが 中身は 空★＋下に 案内を 出す。 */
  var SANTEI_COLS=['①被保険者整理番号','②被保険者氏名','③生年月日','⑤従前の標準報酬月額(健保)','⑤従前(厚年)','⑥従前改定月','⑦昇（降）給','⑧遡及支払額',
    '4月 ⑩日数','4月 ⑪通貨','4月 ⑫現物','4月 ⑬合計','5月 ⑩日数','5月 ⑪通貨','5月 ⑫現物','5月 ⑬合計','6月 ⑩日数','6月 ⑪通貨','6月 ⑫現物','6月 ⑬合計',
    '⑭総計','⑮平均額','⑯修正平均額','決定 標準報酬(健保)','等級(健保)','決定 標準報酬(厚年)','等級(厚年)','⑰個人番号［基礎年金番号］','⑱備考'];
  function santeiRows(recs, year, emps){
    ...
    var byEmp={}; (recs||[]).forEach(function(r){ var m=parseInt(String(r.ym).slice(5,7),10); if(m<4||m>6)return; var d=r.data||{}; (byEmp[r.employee_id]||(byEmp[r.employee_id]={}))[m]={ days:num(d.paymentDays), pay:num(d.shikyuTotal),
      /* ★⑫現物★＝印を 付けた 支給項目の 合計（★印が 無ければ 0＝今までと 同じ★）。⑪通貨＝⑬合計−⑫現物。 */
      genbutsu:num(d.genbutsuTotal) }; });
    return (emps||[]).filter(function(e){ return isActiveInMonth(e, year+'-07'); }).map(function(e){ // 算定基礎日=7/1の被保険者(退職済/退職日が7月前=除外)。退職判定は在籍単一ソースに統一
      var mo=byEmp[e.id]||{}, months=[4,5,6].map(function(m){ var x=mo[m]||{days:0,pay:0,genbutsu:0}; x.genbutsu=num(x.genbutsu); x.tsuka=num(x.pay)-x.genbutsu; return x; }), rule=santeiRule(e);
  ...
  /* ★1行の 中身は ここ 1か所で 作る★（2026-09-03）＝Excel と 画面の 表で 別々に 書くと ★必ず ズレる★。
     並びは SANTEI_COLS と ★同じ順★（列名と 中身が 1本の 一覧から 出る）。
     ★⑰個人番号は いつも 空★（持たない＝裁定 甲）／★⑯修正平均額は 今は 出さない（空）★。 */
  function santeiCells(x, opt){
  ...
  function santeiAoa(rows, year){
    var aoa=[SANTEI_COLS]; rows.forEach(function(x){ aoa.push(santeiCells(x, { yen:false })); });
    return aoa;
  }
  function santeiHTML(rows, year){
    /* ★空欄を 黙って 出さない★（2026-09-03 指示役の裁定＝D-3-1）
       ★⑰個人番号は うちが 持たない★（★持たない物は 漏れない★／マイナンバーを 持つと 保管・持ち出し・
       消し方の 決まりが 丸ごと 付いてくる＝★今日 決める 話では ない★・将来の 電子申請の 時に 司さんの決め）。
       ⇒★何も 言わずに 空だと「アプリの 抜け」に 見える★ので ★紙に そう 書く★。
       ★様式に「⑰個人番号［基礎年金番号］」と 在る★＝基礎年金番号でも よい事も 添える。 */
    var note='<p class="hint" style="margin:0 0 10px">確定済みの<b>'+year+'年4〜6月</b>の明細（総支給・支払基礎日数）から標準報酬を決定（'+year+'年9月〜適用）。'
      +'<br><b>⑰個人番号［基礎年金番号］は このアプリでは お預かりしません</b>（大切な番号なので 持たない作りです）。'
      +'<b>提出前に 手で お書きください</b>（基礎年金番号でも かまいません）。'
      +'<br>⑥従前改定月・⑧遡及支払額は 分かる方だけ 入れてください（空のままでも 出せます）。</p>';
    ...
    var body=withData.map(function(x){
      var cells=santeiCells(x, { yen:true });   /* ★列名と 中身は 同じ 1本から★（2026-09-03） */
      return '<tr>'+cells.map(function(c,i){ return '<td class="'+(i>=3&&i<=16?'num':'')+'">'+esc(String(c))+'</td>'; }).join('')+'</tr>';
    }).join('');
    /* ★丸めた事を 紙にも 出す★（2026-09-03 指示役）＝CSV だけ 9999999 に して 表が 12,000,000 のままだと
       見比べた人が 混乱する。★CSV と 紙で 同じ 文★を 使う（TodokedeCsv.santeiWarn）。 */
    var maru=[];
  ...
  /* ★電子申請（CSV）★（2026-09-03 指示役の裁定＝作るのは「ファイルを 落とす所まで」）
     ・★聞くのは 4問だけ★（12項目ぶん）＝★どこに 書いてあるかを 一緒に 出す＝探させない★
     ・★もう 在る 3項目（所在地・名称・県）は 聞かない★
     ・値は ★会社に 1つ★（state.company）＝倉庫にも 乗る＝端末を 替えても 続く */
  function santeiCsvInput(x, year){
    var c=state.company||{}, e=x.emp||{}, m=x.months||[];
    var sk=(window.TodokedeCsv&&TodokedeCsv.splitSeiriKigou)?TodokedeCsv.splitSeiriKigou(c.seiriKigou):null;
    return {
      jimusho:{ todofuken:(window.TodokedeCsv?TodokedeCsv.KEN_CODE[c.pref]:'')||'',
        gunshiku:sk?sk.gunshiku:'', kigou:sk?sk.kigou:'', jigyoshoNo:c.jigyoshoNo||'',
      ...
      rule: santeiRule(e),
      /* ★備考欄（公式の 項目表 csv225.pdf で 番号を 確かめた）★
         ★6＝短時間労働者／7＝パート★（15日で 算定した 根拠を 示す）
         ★1＝70歳以上被用者算定は 付けられない★＝基礎年金番号が 要る（お預かりしない）
           ⇒★出さずに 名前を 挙げる★（TodokedeCsv.dasuKa が false に なる） */
      bikou: { tanjikan: stType(e) === 'tanjikan', part: stType(e) === 'part',
        over70: isOver70(e, year + '-07') }
    };
  }
  ...
  function jimushoToi(){
    ...
    var fi=function(k,ph,hint){ return '<div class="frow"><div class="flabel">'+hint+'</div>'
      +'<input class="finput" data-fc="'+k+'" value="'+attr(c[k]||'')+'" placeholder="'+esc(ph)+'"></div>'; };
    var TOI_NOUNYU = [
  ...
  function santeiCsvBox(rows, year){
    if(!window.TodokedeCsv) return '';
    /* ★聞き取りは jimushoToi 1か所★（写しを 持たない＝片方だけ 直る事故を 止める） */
    var toi=jimushoToi();
    /* ★止める理由を 先に 数える★（出せない字・カンマ・未入力） */
    var tomeru=toi.tarinai.slice();
    rows.forEach(function(x){ tomeru=tomeru.concat(TodokedeCsv.santeiWarn(santeiCsvInput(x, year)).filter(function(w){ return /使えません/.test(w); })); });
    var nigemichi = tomeru.some(function(t){ return /使えません/.test(t); })
      ? '<p class="hint" style="margin:6px 0 0">日本年金機構の 決まりでは <b>2つの 逃げ道</b>が 認められています（例：髙田 六郎）。'
        + '<br>・<b>代替え文字</b>で 出す … 「高田　六郎」（似た 字に する）'
  ...
  function gekkakuRows(recs, emps){
    ...
    var byEmp={}; (recs||[]).forEach(function(r){ if((r.data||{}).kind==='bonus')return; (byEmp[r.employee_id]||(byEmp[r.employee_id]={}))[r.ym]={ days:num((r.data||{}).paymentDays), pay:num((r.data||{}).shikyuTotal), genbutsu:num((r.data||{}).genbutsuTotal), has:true }; });
    return (emps||[]).filter(function(e){ if(!isActiveInMonth(e, state.month))return false; var s=e.shaho||{}; return !!s.henkoYm && (s.fixedChanged || num(s.prevHyojun)>0); }).map(function(e){ // 在籍単一ソースに統一(退職者は随時改定対象外)
      ...
      var mo=byEmp[e.id]||{}, months=yms.map(function(ym){ var d=mo[ym]; return { ym:ym, days:d?d.days:0, pay:d?d.pay:0, genbutsu:d?num(d.genbutsu):0, has:!!d }; });
  ...
  /* ★月額変更届の 電子申請 CSV に 渡す物★（2026-09-04）
     ★算定と 同じ 形で 渡す★＝事業所は 会社情報から、人は 画面が 持っている 3か月から。
     ★渡し忘れが 一番 怖い★ので、変動月・従前の改定月・現物・短時間・70歳は ここで 必ず 入れる。 */
  function gekkakuCsvInput(x){
    var c=state.company||{}, e=x.emp||{}, m=x.months||[], s=e.shaho||{};
    var sk=(window.TodokedeCsv&&TodokedeCsv.splitSeiriKigou)?TodokedeCsv.splitSeiriKigou(c.seiriKigou):null;
    var z=x.z||{}, hp=z.health||{}, pp=z.pension||{};
    return {
      jimusho:{ todofuken:(window.TodokedeCsv?TodokedeCsv.KEN_CODE[c.pref]:'')||'',
        gunshiku:sk?sk.gunshiku:'', kigou:sk?sk.kigou:'', jigyoshoNo:c.jigyoshoNo||'',
        zipOya:String(c.zip||'').replace(/[^0-9]/g,'').slice(0,3), zipKo:String(c.zip||'').replace(/[^0-9]/g,'').slice(3,7),
  ...
  /* ★落とす箱★（★出せない人は 名前を 挙げて 知らせ、ファイルに 入れない★＝算定と 同じ 決め） */
  function gekkakuCsvBox(rows){
    if(!window.TodokedeCsv) return '';
    var maru=[]; rows.forEach(function(x){ maru=maru.concat(TodokedeCsv.gekkakuWarn(gekkakuCsvInput(x))); });
    var deru=rows.filter(function(x){ return TodokedeCsv.dasuKaGekkaku(gekkakuCsvInput(x)); });
    return '<div class="card" style="margin-top:10px"><div class="card-h">電子申請用の CSV（年金事務所へ）</div>'
      +'<p class="hint" style="margin:0 0 8px">月額変更届は <b>3か月とも 支払基礎日数が 17日以上</b>'
      +'（短時間労働者は 11日以上）でないと 出せません（日本年金機構の 決まり）。</p>'
      +(maru.length?('<div class="cr-warn" style="margin:8px 0 0">⚠ <b>この方は 入れていません</b><br>'
        +maru.map(esc).join('<br>')+'</div>'):'')
      +'<p class="hint" style="margin:8px 0 0">ファイル名は <b>SHFD0006.CSV</b>（電子申請の 決まり）。</p>'
      +'<div style="margin-top:10px"><button class="btn-primary" id="b-gekkaku-csv"'+(deru.length?'':' disabled')+'>'
      +(deru.length?('CSVを作る（'+deru.length+'人・SHFD0006.CSV）'):'出せる人が いません')+'</button></div></div>';
  }
  ...
  /* ★労災保険率は「業種を 選ぶ」＝手で 打たせない★（2026-09-04）
     ★出どころ★ 労働保険の保険料の徴収等に関する法律施行規則 ★別表第１（労災保険率表）★
       （e-Gov 法令検索から 機械で 取った＝kyuyo/lib/rousai-ritsu.js・★53業種★）
     ★2026-09-04 まで★ 会社が ‰ を 手で 打ち込むだけ＝★間違えても 誰も 気づけない★
       （app.js の 古い 注記「労災率…部品も 一次情報も 無い」は ★もう 嘘★＝直した）
     ★決め方は 1か所★:
  ...
  function rousaiPermilOf(c){
    c=c||{};
    var RR=window.RousaiRitsu;
    if(c.rousaiShurui && RR){ var p=RR.permilOf(c.rousaiShurui); if(p!=null) return p; }
    var te=num(c.rousaiRate);
    return te>0?te:null;
  }
  function roudouGokei(rousaiWage, koyoWage, rousaiRate, koyoRate){
    /* ★計算は lib に 1か所★（kyuyo/lib/roudou-shinkoku.js＝原文を 全部 写してある） */
  ...
  function roudouSummary(recs, fy, emps){
    ...
    /* ★申告書に 書く 数字★（原文は kyuyo/lib/roudou-shinkoku.js の 頭）
       ・一般拠出金＝賃金総額×0.02/1000（★確定のみ・概算払い 無し★）
    ...
    if(RS && g.gokei!=null){
      ...
      var katahou=!(rousaiPermil>0 && koyoFull!=null);
      enno=RS.ennoDekiruka(gaisan.gaku, katahou);
      if(enno.ok) kibetsu=RS.ennoKibetsu(gaisan.gaku, 3);
    }
    /* ★精算＝確定 −（前年度に 納めた 概算）★
  ...
  function roudouAoa(sum, fy){
    ...
    aoa.push([]);
    aoa.push(['【申告書に 書く 数字】']);
    aoa.push(['① 確定保険料（労災＋雇用）', sum.gokeiRyo!=null?sum.gokeiRyo:'（率が そろっていません）']);
    aoa.push(['② 一般拠出金（賃金総額×0.02/1000・確定のみ）', sum.ippan?sum.ippan.gaku:'—']);
    aoa.push(['', '※ 一般拠出金の 賃金総額は 労災の 算定基礎額を 使っています（申告書の 該当ページが 画像で 読めていません）']);
    aoa.push(['③ 概算保険料（今年度）', sum.gaisan?sum.gaisan.gaku:'—']);
    aoa.push(['', sum.gaisan?('※ '+sum.gaisan.base.riyu+'／見込額を 別に 出す 場合は 労働局の 用紙で ご確認ください'):'']);
    aoa.push(['④ 延納（3回に 分ける）', sum.enno?(sum.enno.ok?'できます':'できません'):'—']);
    if(sum.enno) aoa.push(['', '※ '+sum.enno.riyu]);
    if(sum.kibetsu){
      aoa.push(['　第1期（端数を 含む）', sum.kibetsu.ki[0]]);
      aoa.push(['　第2期', sum.kibetsu.ki[1]]);
      aoa.push(['　第3期', sum.kibetsu.ki[2]]);
      aoa.push(['', '※ 分けた 端数は すべて 第1期に 合算します（国等の債権債務等の金額の端数計算に関する法律 第三条）']);
    }
    if(sum.seisan && sum.seisan.measured){
      aoa.push(['⑤ 前年度に 納めた 概算保険料', num((state.company||{}).zennendoGaisan)]);
      aoa.push(['　精算', sum.seisan.kubun==='fusoku'?('不足 '+sum.seisan.gaku+'円')
        :sum.seisan.kubun==='amari'?('多く 納めています '+sum.seisan.gaku+'円')
        :'ちょうど']);
      if(sum.seisan.kubun==='amari'){
        aoa.push(['', '※ 多く 納めた分は 今年度の 概算保険料に 充てられます。'
          +'お金で 戻す 場合は 申告書だけでは 戻りません＝「労働保険料・一般拠出金還付請求書」を 別に 出してください']);
      }
    }
    aoa.push([]);
    ...
    var ryoBox='<div class="card" style="margin-top:10px"><div class="card-h">保険料の概算</div>'
      ...
      +'<div><div class="hint">雇用保険（'+esc(gLabel)+'・'+koyoRateTxt+'）</div><b style="font-size:16px">'+(sum.koyoRyo!=null?yen(sum.koyoRyo):'—')+'</b></div>'
        ...
        +((window.RousaiRitsu?RousaiRitsu.list():[]).map(function(r){
            return '<option value="'+esc(r.shurui)+'"'+((state.company||{}).rousaiShurui===r.shurui?' selected':'')+'>'
              +esc(r.bunrui+'／'+r.shurui)+'（'+r.permil+'‰）</option>'; }).join(''))
        +'<option value="船舶所有者の事業"'+((state.company||{}).rousaiShurui==='船舶所有者の事業'?' selected':'')+'>船舶所有者の事業（42‰）</option>'
        +'<option value="__te__"'+(((state.company||{}).rousaiShurui==='__te__')?' selected':'')+'>一覧に無い（率を 自分で 入れる）</option>'
        +'</select>'
        +(((state.company||{}).rousaiShurui==='__te__')
          ? ('<span style="display:inline-flex;align-items:center;gap:6px;margin-left:8px"><input class="finput num" data-rousai-rate="1" inputmode="decimal" style="width:80px" value="'+esc(String((state.company||{}).rousaiRate||''))+'"><span class="hint">‰</span></span>')
          : '')
        +'<div style="margin-top:4px"><b id="roudou-rousai-ryo" style="font-size:16px">'+(sum.rousaiRyo!=null?yen(sum.rousaiRyo):'—')+'</b></div></div>'
      +'<div><div class="hint">一般拠出金（石綿）</div><b style="font-size:16px">'
        +(sum.ippan?yen(sum.ippan.gaku):'—')+'</b>'
        +'<div class="hint2" style="margin-top:2px">賃金総額×0.02/1000（確定のみ・概算払いは ありません）</div>'
        /* ★未測定を 画面にも 出す★（2026-09-04 指示役＝Excelを 開かない 人には 伝わらない） */
        +'<div class="hint2" style="margin-top:2px">※ 賃金総額は <b>労災と 同じ額</b>で 計算しています。'
        +'ここは まだ 原文で 確かめられていません（申告書の 該当ページが 画像のため）。</div></div>'
  ...
  function shikakuRows(emps){
    ...
    (emps||[]).forEach(function(e){ var yakuin=(e.payType==='役員'); // 役員も健保・厚年の被保険者=取得/喪失の対象(備考で区別)
      ...
      if(e.joinYmd){ var notesA=[]; if(yakuin)notesA.push('役員'); if(isKaigoNow(e,e.joinYmd))notesA.push('介護保険 第2号'); if(stLabel(e))notesA.push(stLabel(e)); if(isOver70(e,String(e.joinYmd).slice(0,7)))notesA.push('70歳以上');
        rows.push({ kind:'取得', emp:e, name:e.name||'', birthYmd:e.birthYmd||'', date:e.joinYmd, decH:decH?decH.hyojun:0, decHGrade:decH?decH.tokyu:0, decP:decP?decP.hyojun:0, decPGrade:decP?decP.tokyu:0, reason:'', note:notesA.join('／') }); }
      ...
      if(e.taishokuYmd){ var lossYmd=ymdPlus1(e.taishokuYmd); var notesL=[]; if(yakuin)notesL.push('役員'); if(isOver70(e,String(e.taishokuYmd).slice(0,7)))notesL.push('70歳以上');
        rows.push({ kind:'喪失', emp:e, name:e.name||'', birthYmd:e.birthYmd||'', date:lossYmd, decH:0, decHGrade:0, decP:0, decPGrade:0, reason:'退職等', note:notesL.join('／') }); }
      else if(e.retired){ var notesR=[]; if(yakuin)notesR.push('役員'); notesR.push('退職日を入力してください'); // retired=trueだが退職日未入力→喪失届が漏れないよう要入力で表示
        rows.push({ kind:'喪失', emp:e, name:e.name||'', birthYmd:e.birthYmd||'', date:'', decH:0, decHGrade:0, decP:0, decPGrade:0, reason:'退職等', note:notesR.join('／') }); }
    });
    return rows;
  }
  ...
  /* ★資格取得届の 電子申請 CSV に 渡す物★（2026-09-05）
     ★算定・月変・賞与と 同じ 形★＝事業所は 会社情報から、人は 従業員マスタから。
     ★このアプリは マイナンバーを お預かりしない★ので、年金機構の 相関チェックで
     郵便番号・住所（カナ）が 必須に なる（項番29・30・31）。 */
  function shutokuCsvInput(x){
    var c=state.company||{}, e=x.emp||{};
    var sk=(window.TodokedeCsv&&TodokedeCsv.splitSeiriKigou)?TodokedeCsv.splitSeiriKigou(c.seiriKigou):null;
    var sb=shahoBasisOf(e), hoshu=(sb&&sb.hoshu>0)?sb.hoshu:0;
    return {
      ...
      shutokuYmd: x.date||'',
      /* ★今日は 画面が 渡す★（lib は 時計を 持たない＝headless の 決まり）
         原文＝「『資格取得年月日』≦『システムチェック実施日』であること」 */
      kyou: new Date().toISOString().slice(0,10),
      tsuka: hoshu, genbutsu: 0,
      bikou:{ over70: isOver70(e, String(x.date||'').slice(0,7)),
        fuyo: Number(e.fuyou||0)>0,
        nijo: false,
        tanjikan: !!stType(e),
        saikoyo: false }
    };
  }
  /* ★出せる／出せないを 実物から 出す箱★（★出来ていない物のボタンを 見せない★） */
  function shutokuCsvBox(rows){
    if(!window.TodokedeCsv||!TodokedeCsv.shutokuRow) return '';
    var toku=(rows||[]).filter(function(x){ return x.kind==='取得'; });
    if(!toku.length) return '';
    var maru=[]; toku.forEach(function(x){ maru=maru.concat(TodokedeCsv.shutokuWarn(shutokuCsvInput(x))); });
    var deru=toku.filter(function(x){ return TodokedeCsv.dasuKaShutoku(shutokuCsvInput(x)); });
    /* ★事業所の 聞き取りは 算定と 同じ 1か所★（写しを 持たない＝片方だけ 直る事故を 止める） */
    var toi=jimushoToi();
    var tomeru=toi.tarinai.slice();
    return '<div class="card" style="margin-top:10px"><div class="card-h">電子申請用の CSV（資格取得届・年金事務所へ）</div>'
      +'<p class="hint" style="margin:0 0 8px">資格取得届は <b>入社から5日以内</b>に出します。'
      +'このアプリは <b>マイナンバーをお預かりしません</b>ので、代わりに'
      +' <b>郵便番号と住所（カナ）</b>が要ります（日本年金機構の決まり）。</p>'
      +toi.toi
      +(maru.length?('<div class="cr-warn" style="margin:8px 0 0">⚠ <b>この方は 入れていません</b><br>'
        +maru.map(esc).join('<br>')+'</div>'):'')
      +(tomeru.length?('<div class="cr-warn" style="margin:8px 0 0">⚠ <b>まだ 出せません</b><br>'+tomeru.map(esc).join('<br>')+'</div>')
        :'<p class="hint" style="margin:8px 0 0">ファイル名は <b>SHFD0006.CSV</b>（電子申請の 決まり）。'
          +'この ファイルを e-Gov で 出します（送るのは お客さまです）。</p>')
      +'<div style="margin-top:10px"><button class="btn-primary" id="b-shutoku-csv"'+((deru.length&&!tomeru.length)?'':' disabled')+'>'
      +(tomeru.length?('CSVを作る（'+tomeru.length+'件 直してから）')
        :(deru.length?('CSVを作る（'+deru.length+'人・SHFD0006.CSV）'):'出せる人が いません'))+'</button></div></div>';
  }
  /* ★資格喪失届の 電子申請 CSV に 渡す物★（2026-09-05）
     ★喪失日＝退職日の 翌日★（原文 項番14「原因が'4'（退職等）→ 資格喪失年月日＝退職日死亡日＋１日」）
     ＝うちの shikakuRows が もう ymdPlus1 で 出している（突き合わせ済み） */
  function soshitsuCsvInput(x){
    var c=state.company||{}, e=x.emp||{};
    var sk=(window.TodokedeCsv&&TodokedeCsv.splitSeiriKigou)?TodokedeCsv.splitSeiriKigou(c.seiriKigou):null;
    return {
      jimusho:{ todofuken:(window.TodokedeCsv?TodokedeCsv.KEN_CODE[c.pref]:'')||'',
        ...
        tel1:String(c.tel||'').split('-')[0]||'', tel2:String(c.tel||'').split('-')[1]||'', tel3:String(c.tel||'').split('-')[2]||'' },
      emp:{ seiriNo:e.hokenshaNo||'', kana:e.kana||e.furiKana||'', kanji:e.name||'', birthYmd:e.birthYmd||'',
        kisoNenkin:e.kisoNenkin||'' },
      taishokuYmd: e.taishokuYmd||'',
      soshitsuYmd: x.date||'',                 /* ★退職日の 翌日★（shikakuRows が 作る） */
      gen: (window.TodokedeCsv&&TodokedeCsv.SOSHITSU_GEN)?TodokedeCsv.SOSHITSU_GEN.taishoku:'4',
      /* ★今日は 画面が 渡す★（lib は 時計を 持たない） */
      kyou: new Date().toISOString().slice(0,10),
      bikou:{ fugaito: isOver70(e, String(x.date||'').slice(0,7)) }
    };
  }
  /* ★出せる／出せないを 実物から 出す箱★ */
  function soshitsuCsvBox(rows){
    if(!window.TodokedeCsv||!TodokedeCsv.soshitsuRow) return '';
    var sos=(rows||[]).filter(function(x){ return x.kind==='喪失'; });
    if(!sos.length) return '';
    var maru=[]; sos.forEach(function(x){ maru=maru.concat(TodokedeCsv.soshitsuWarn(soshitsuCsvInput(x))); });
    var deru=sos.filter(function(x){ return TodokedeCsv.dasuKaSoshitsu(soshitsuCsvInput(x)); });
    var toi=jimushoToi();
    var tomeru=toi.tarinai.slice();
    return '<div class="card" style="margin-top:10px"><div class="card-h">電子申請用の CSV（資格喪失届・年金事務所へ）</div>'
      +'<p class="hint" style="margin:0 0 8px">資格喪失届は <b>退社から5日以内</b>に出します。'
      +'<b>資格喪失日は 退職日の翌日</b>です（日本年金機構の決まり）。'
      +'このアプリは <b>マイナンバーをお預かりしません</b>ので、代わりに'
      +' <b>基礎年金番号</b>（4桁-6桁）が要ります。</p>'
      +'<p class="hint" style="margin:0 0 8px">※ <b>健康保険組合</b>に出す会社は、基礎年金番号での電子申請が'
      +'できません（マイナンバーが要ります）。<b>協会けんぽ</b>なら このまま出せます。</p>'
      +toi.toi
      +(maru.length?('<div class="cr-warn" style="margin:8px 0 0">⚠ <b>この方は 入れていません</b><br>'
        +maru.map(esc).join('<br>')+'</div>'):'')
      +(tomeru.length?('<div class="cr-warn" style="margin:8px 0 0">⚠ <b>まだ 出せません</b><br>'+tomeru.map(esc).join('<br>')+'</div>')
        :'<p class="hint" style="margin:8px 0 0">ファイル名は <b>SHFD0006.CSV</b>（電子申請の 決まり）。'
          +'この ファイルを e-Gov で 出します（送るのは お客さまです）。</p>')
      +'<div style="margin-top:10px"><button class="btn-primary" id="b-soshitsu-csv"'+((deru.length&&!tomeru.length)?'':' disabled')+'>'
      +(tomeru.length?('CSVを作る（'+tomeru.length+'件 直してから）')
        :(deru.length?('CSVを作る（'+deru.length+'人・SHFD0006.CSV）'):'出せる人が いません'))+'</button></div></div>';
  }
  /* ★被扶養者(異動)届（2202700）の 電子申請 CSV★（2026-09-08）
  ...
  /* ★1人の 従業員 → 届書 何枚ぶん か★（配偶者1＋その他2 ずつ 切る） */
  function fuyoInputsOf(e){
    var ks=(e&&e.kazoku||[]).filter(function(k){ return k && (k.kanji||k.kana||k.birthYmd); });
    if(!ks.length) return [];
    var hai=null, sono=[];
    ks.forEach(function(k){ if(String(k.zokugara||'')==='haigusha' && !hai) hai=k; else sono.push(k); });
    var kyou=new Date().toISOString().slice(0,10);
  ...
  function fuyoTodoke(){
    ...
    (state.employees||[]).forEach(function(e){
      if(e.employmentType==='contractor') return;   /* 業務委託に 被扶養者届は 無い */
      fuyoInputsOf(e).forEach(function(inp){ out.push({ emp:e, inp:inp }); });
    });
    return out;
  }
  function fuyoCsvBox(){
    if(!window.TodokedeCsv||!TodokedeCsv.fuyoCsv) return '';
    var all=fuyoTodoke();
    if(!all.length) return '';
    var deru=[], maru=[];
    all.forEach(function(x){
      var d=TodokedeCsv.dasuKaFuyo(x.inp);
      if(d.ok) deru.push(x);
      else d.naze.forEach(function(w){ maru.push((x.emp.name||'（名前なし）')+'…'+w); });
    });
    var toi=jimushoToi();
    var tomeru=toi.tarinai.slice();
    var ninzu=all.reduce(function(a,x){ return a+(x.inp.hai?1:0)+x.inp.sonota.length; },0);
    return '<div class="card" style="margin-top:10px"><div class="card-h">電子申請用の CSV（被扶養者(異動)届・年金事務所へ）</div>'
      +'<p class="hint" style="margin:0 0 8px">家族が増えた時は <b>5日以内</b>に出します。'
      +'従業員マスタの <b>詳細設定 ▸ 家族（被扶養者）</b>に入れた方が出ます'
      +'（今 <b>'+ninzu+'人</b>・届書 <b>'+all.length+'枚</b>）。<br>'
      +'1枚に入るのは <b>配偶者1人＋その他2人まで</b>なので、多い時は自動で分けます（日本年金機構の決まり）。<br>'
      +'このアプリは <b>マイナンバーをお預かりしません</b>ので、代わりに'
      +' <b>基礎年金番号と 郵便番号・住所</b>が要ります。</p>'
      +toi.toi
      +(maru.length?('<div class="cr-warn" style="margin:8px 0 0">⚠ <b>この方は 入れていません</b><br>'
        +maru.slice(0,12).map(esc).join('<br>')
        +(maru.length>12?('<br>…ほか '+(maru.length-12)+'件'):'')+'</div>'):'')
      +(tomeru.length?('<div class="cr-warn" style="margin:8px 0 0">⚠ <b>まだ 出せません</b><br>'+tomeru.map(esc).join('<br>')+'</div>')
        :'<p class="hint" style="margin:8px 0 0">ファイル名は <b>SHFD0006.CSV</b>（電子申請の 決まり）。'
          +'この ファイルを e-Gov で 出します（送るのは お客さまです）。</p>')
      +'<div style="margin-top:10px"><button class="btn-primary" id="b-fuyo-csv"'+((deru.length&&!tomeru.length)?'':' disabled')+'>'
      +(tomeru.length?('CSVを作る（'+tomeru.length+'件 直してから）')
  ...
  function renderShikaku(sub){ var host=$('#view-cho'); state._shikakuRows=shikakuRows(state.employees);
    /* ★家族の届出は 入社退社と 別の話★＝資格の行が 0本でも この箱は 出す */
    host.innerHTML=sub+todokedeIchiranHTML()+shikakuHTML(state._shikakuRows)+fuyoCsvBox(); }
  ...
  var TDK_LIST = [
    ...
    /* ★2026-09-08 出せるように なった★＝土台(fuyoRow/fuyoCsv)＋家族を入れる画面＋保管＋見張り。
       ★今 出せるのは「増えた（該当）」だけ★＝減った・変わったは まだ 画面が 無い。
       CSVを 作るのは fuyoCsv、材料が 揃っているかは dasuKaFuyo が 見る。 */
    { itsu:'家族が増えた時（5日以内）',               nm:'被扶養者(異動)届・国民年金第3号', yoshiki:'2202700', tsukuru:'fuyoCsv' },
  ...
  function payDateForSlip(){ var c=(state.company&&state.company.payCycle)||'monthly';
    ...
    return payDateStr(); }
  function buildPeople(emps){ return emps.map(function(e){ var r=compute(e); var k=(e.kintai||[]).filter(function(x){ if(/代休取得|振替休日/.test(x.label||'')) return num(x.value)>0; return true; }); var oi=k.findIndex(function(x){return /出勤/.test(x.label||'');}); var wt={label:'労働時間',value:workedLabel(e)}; if(oi>=0)k.splice(oi+1,0,wt); else k.unshift(wt); var _p={ name:e.name, company:state.company.name, payDate:payDateForSlip(), kintai:k, shikyu:r.shikyu, kojo:r.kojo, net:r.net, shikyuTotal:r.shikyuTotal, kojoTotal:r.kojoTotal, warnings:empWarnings(e),
      /* ★県が 未選択の人は 紙にも 仮計算だと 書く★（黙って 東京の率で 出さない・2026-09-03）
         ★札が 無い時は 欄そのものを 作らない★＝凍結した 見本（ops-golden-parity）と 1バイトも 変えない為。 */
      };
    var _kari=kariNote(e); if(_kari) _p.kari=_kari; return _p; }); }
  ...
  /* ★読めない年月を そのまま 紙に 焼かない★（2026-09-06 実測）
     payYm に 年-月 以外が 入っていると お客さんの 紙に
     「令和982年0月賞与」と 出た（値は "300000"）。
     見える 選択肢からは 壊れた値に ならないが、古い控えや 前の版から 戻すと 起き得る。
     ★読めない時は 対象月に 落とす★（嫌な 数字を 従業員に 見せない）。 */
  function bonusYmSafe(){ var ym=String(bonusYmOf()||'');
    if(!/^\d{4}-(0[1-9]|1[0-2])$/.test(ym)) ym=String(state.month||'');
    return /^\d{4}-(0[1-9]|1[0-2])$/.test(ym)?ym:''; }
  function bonusMonthLabel(){ var ym=bonusYmSafe(); if(!ym) return '賞 与';
    var y=Number(ym.slice(0,4)), m=Number(ym.slice(5,7)); var k=['','一','二','三','四','五','六','七','八','九','十','十一','十二'];
    return '令 和 '+(y-2018)+' 年 '+(k[m]||m)+' 月 賞 与'; }
  /* ★賞与の 紙の 支給日を 月次と 同じ 書き方に★（2026-09-06 実測）
     支給日の欄は 2026-09-03 に 「例 2026-12-10」の形で 受け取るように 変えた（銀行へ 送る為）。
     その後 賞与の紙だけが 生の「2026-12-10」のままに なっていた。
     月次の紙は「令和8年10月25日」――★同じ会社の 紙で 書き方が 食い違っていた★。
     ★入れてもらった 自由文は 消さない★＝日付と 読めた時だけ 和暦にする。 */
  function bonusPayDateStr(){ var v=String((state.bonus&&state.bonus.payDay)||'').trim();
    if(!v) return payDateStr();
    var d=/^(\d{4})-(\d{1,2})-(\d{1,2})$/.exec(v);
    return d?('令和'+(Number(d[1])-2018)+'年'+Number(d[2])+'月'+Number(d[3])+'日'):v; }
  function buildBonusPeople(emps){ return emps.map(function(e){ var c=computeBonus(e);
    ...
    return { name:e.name, company:state.company.name, payDate:bonusPayDateStr(), kintai:[], shikyu:shikyu, kojo:kojo, net:c.net, shikyuTotal:c.totalGross, kojoTotal:c.si.total+c.taxAmt+c.addKojoTotal }; }); }
  ...
  function furikomiGate(transfers, committer){
    ...
    var noDate=!/^[0-9]{4}$/.test(String(furiMMDDFor(furiMode())||''));
    return {
      zengin:{ enabled:ready.length>0 && !cm && !noDate, count:ready.length,
  ...
  /* ★賞与も 振込に 出す★（2026-09-03 指示役の裁定＝甲）
     ＝★同じ道を 使う★（人の 絞り方と 口座の 見方は 月次と 同じ／金額と 対象月だけ 差し替える）。
     ★新しい 仕組みを 作らない★＝ready の 決め方も 全銀の 組み立ても そのまま。 */
  function furiMode(){ return (state.printMode==='bonus') ? 'bonus' : 'monthly'; }
  function buildTransfers(mode){
    mode = mode || furiMode();
    var bonus = (mode==='bonus');
    var ym = bonus ? bonusYmOf() : state.month;
    return state.employees.filter(function(e){
      if(!isActiveInMonth(e,ym) || e.retired) return false;
  ...
  /* ★モード別の 取組日★＝月次は 会社の 支給日から／賞与は ★賞与の 支給日★から（読めた時だけ）。
     ★門番は D-1 と 同じ考え★＝読めなければ 空＝★1バイトも 作らない★。 */
  function furiMMDDFor(mode){
    if(mode==='bonus') return PM().bonusMMDD({ bonus:state.bonus, company:state.company });
    return furiMMDD(state.month);
  }
  /* ★全銀の 種別コード★（一次情報 2026-09-03＝21:総合／11:給与／12:賞与・★22は 無い★）
     ★どちらの 契約かは 会社ごと★＝設定から。★既定は 総合振込（21）＝今のまま★。 */
  function furiShubetsuFor(mode){
    var keiyaku=String((state.company&&state.company.furiKeiyaku)||'sogo');
    if(keiyaku!=='kyuyo') return '21';                 /* 総合振込＝月次も 賞与も 21 */
    return (mode==='bonus') ? '12' : '11';             /* 給与振込の 契約＝給与11／賞与12 */
  }
  /* ★賞与の 振込指定日の 箱★（2026-09-03）＝★D-1 と 同じ 見え方★に 揃える。
     賞与の 支給日は ★自由文★なので、★日付として 読めた時だけ★ 使う。
     ★元の 文字は 1文字も 消さない★（画面にも そのまま 出す）。 */
  function furiDateBoxBonus(){
    var b=state.bonus||{}, raw=String(b.payDay||'').trim();
    var o=PM().bonusDateObj({ bonus:b, company:state.company });
    var w=['日','月','火','水','木','金','土'];
    if(!o){
      return '<div class="frow"><div class="flabel">振込指定日<span class="hint2">賞与</span></div>'
        +'<div class="cr-warn" style="margin:0">賞与の振込指定日を決められませんでした。'
        +(raw?('いま入っているのは「'+esc(raw)+'」です。'):'賞与の「支給日」が空です。')
        +'銀行に送るには <b>2026-12-10</b> の形（年-月-日）が要ります。'
        +'<div style="margin-top:6px"><b class="mw-fix" data-fix-bonus-payday>賞与の 支給日を 入れる ▸</b></div></div></div>';
    }
    var p2=function(n){return (n<10?'0':'')+n;};
    var dt=new Date(o.y,o.m-1,o.d);
    var head=o.y+'年'+o.m+'月'+o.d+'日（'+w[dt.getDay()]+'）';
    var why='賞与の支給日「'+esc(raw)+'」から出しました。'
      +(o.moved?(esc(o.reason)+'で銀行が休みなので '+(o.shift==='next'?'次':'前')+'の営業日にしています。'):'');
    return '<div class="frow"><div class="flabel">振込指定日<span class="hint2">賞与</span></div>'
      +'<div><div class="bx-val" style="font-weight:700;white-space:nowrap;font-size:16px">'+esc(head)+'</div>'
      +'<p class="hint" style="margin:2px 0 6px">'+why+'</p></div></div>';
  }
  function furiDateBox(){
    var c=state.company||{};
    var mode=furiMode();
    if(mode==='bonus'){ return furiDateBoxBonus(); }
    var d=furiDateOf(state.month);
    var w=['日','月','火','水','木','金','土'];
    if(!d){
      /* ★丙（2026-09-03 指示役の裁定）＝★聞く★★
         文で「入れてください」と 言うだけだと ★人が 自分で 探す★。
         穴①で 作った ★押せる案内★と 同じ形に して、★支給日の 入れる所★まで 連れて行く。
  ...
  function renderFuri(){
    ...
    /* ★銀行との 契約を 聞く★（2026-09-03 指示役の裁定＝甲’）
       ...
       ★どちらの 契約かは 会社ごと★＝アプリが 決められない ⇒★聞く（既定は 今のまま＝総合振込）★。
       ★専門用語を そのまま 出さない★＝「種別コード」ではなく「銀行とは どちらの 契約ですか？」。
       ★71/72 も 規定には 在るが 使わない★＝SMBC の 資料は 11/12 しか 書いていない＝★広く 通る方★。 */
    var keiyakuNow=String((c.furiKeiyaku||'sogo'));
    var keiyaku='<div class="sec-lb" style="margin-top:0">銀行とは どちらの 契約ですか？</div>'
      +'<div class="frow"><select class="finput" data-fc="furiKeiyaku">'
      +'<option value="sogo"'+(keiyakuNow!=='kyuyo'?' selected':'')+'>総合振込（ふつうはこちら）</option>'
      +'<option value="kyuyo"'+(keiyakuNow==='kyuyo'?' selected':'')+'>給与振込・賞与振込の契約をしている</option>'
      +'</select>'
      +'<p class="hint" style="margin:4px 0 0">分からなければ そのままで大丈夫です。'
      +'銀行に「給与振込」の契約をしている会社だけ 下を選んでください（ファイルの中の 種別が 変わります）。</p></div>';
    var committer=keiyaku+'<div class="sec-lb" style="margin-top:0">委託者情報（自社・銀行から通知される値）</div>'
    ...
    /* ★賞与の 時は 賞与だと 書く★（2026-09-03）＝前は 賞与でも「2026-09・差引支給額」と 月次の 見出しだった
       （★どの お金を 送るのか 読んだ人に 分からない★）。 */
    var listHTML='<div class="sec-lb">振込対象（'
      +esc(furiMode()==='bonus' ? (bonusYmOf()+'・賞与の 差引支給額') : (state.month+'・差引支給額'))+'）</div>';
    if(!tr.length){ listHTML+='<p class="hint" style="margin:0">'
      +(furiMode()==='bonus' ? '賞与の金額を入れた人がいません（「入力 ▸ 賞与」で入れてください）。' : '対象月に在籍する従業員がいません。')
      +'</p>'; }
    else {
  ...
  function bind(){
    ...
    document.addEventListener('click',function(e){ var hi=e.target.closest('.help-i'); if(hi){ openHelp(hi.dataset.help); return; } if(e.target.closest('[data-taishoku-calc]')){ openTaishokuCalc(null); return; } if(e.target.closest('[data-onboard-close]')){ state.onboardDone=true; renderOnboard(); if(window.persistSaveDebounced)persistSaveDebounced(); return; }
      var gob=e.target.closest('[data-onboard-goto]'); if(gob){ var g=gob.dataset.onboardGoto; // はじめかたガイド: 未完ステップ→その画面へジャンプ
        ...
        return; }
      /* ★丙＝支給日の 入れる所まで 連れて行く★（2026-09-03）。穴①と 同じ「押せる案内」。 */
      /* ★賞与の 支給日へ 連れて行く★（2026-09-03）＝穴①と 同じ「押せる案内」。 */
      /* ★電子申請用の CSV を 作って 落とす★（2026-09-03）
         ★媒体通番は「落とした時」に 上げる★（作り直す たびに 飛ばさない）／★1件も 作れない時は 上げない★ */
      ...
      if(e.target.closest('#b-soshitsu-csv')){
        if(!window.TodokedeCsv||!TodokedeCsv.soshitsuRow){ uiAlert('部品が読めていません。開き直してください。'); return; }
        var sr2=(state._shikakuRows||[]).filter(function(x){ return x.kind==='喪失'; });
        if(!sr2.length){ uiAlert('資格喪失の 対象者が いません（従業員マスタの 退職日を 入れてください）。'); return; }
        var si3=sr2.filter(function(x){ return TodokedeCsv.dasuKaSoshitsu(soshitsuCsvInput(x)); });
        if(!si3.length){ uiAlert('電子申請に 出せる人が いませんでした。'+String.fromCharCode(10)
          +'退職日・氏名（カナ）・基礎年金番号（4桁-6桁）が 要ります。'); return; }
        var sco2=state.company||{};
        var srows2=si3.map(function(x){ return TodokedeCsv.soshitsuRow(soshitsuCsvInput(x)); });
        var stsu2=Number(sco2.baitaiTsuban||0);
        var sf2=TodokedeCsv.soshitsuCsv({ jimusho:soshitsuCsvInput(si3[0]).jimusho,
          baitai:{ tsuban:TodokedeCsv.nextTsuban(stsu2), ymd:new Date().toISOString().slice(0,10) },
          kyou:new Date().toISOString().slice(0,10), rows:srows2 });
        if(sf2.kensa && sf2.kensa.errors.length){
          uiAlert('年金機構の 決まりに 合わない所が '+sf2.kensa.errors.length+'件 あります。'
            +String.fromCharCode(10)+sf2.kensa.errors.slice(0,5).map(function(x){
              return (x.gyo)+'人目 項番'+x.no+' '+x.name+'／'+x.why; }).join(String.fromCharCode(10)));
          return;
        }
        if(!sf2.bytes.length){ uiAlert('出せる人がいませんでした（1バイトも作っていません）。'); return; }
        if(sf2.tooBig){
          uiAlert('このファイルは 4.5MB 以上になるため、電子申請できません。'
            +String.fromCharCode(10)+'人数を分けて 出してください（日本年金機構の決まりです）。');
          return;
        }
        dlBytes(sf2.bytes, sf2.name, 'text/csv');
        state.company.baitaiTsuban=TodokedeCsv.nextTsuban(stsu2);
        persistSaveDebounced();
        return;
      }
      if(e.target.closest('#b-fuyo-csv')){
        if(!window.TodokedeCsv||!TodokedeCsv.fuyoCsv){ uiAlert('部品が読めていません。開き直してください。'); return; }
        var fa=fuyoTodoke();
        if(!fa.length){ uiAlert('家族（被扶養者）を 入れた 従業員が いません。'+String.fromCharCode(10)
          +'従業員マスタの 詳細設定 ▸ 家族（被扶養者）に 入れてください。'); return; }
        var fi=fa.filter(function(x){ return TodokedeCsv.dasuKaFuyo(x.inp).ok; });
        if(!fi.length){ uiAlert('電子申請に 出せる 届書が ありませんでした。'+String.fromCharCode(10)
          +'氏名（カナ）・生年月日・続柄が 要ります（画面の ⚠ を 見てください）。'); return; }
        var fco=state.company||{};
        var frows=fi.map(function(x){ return TodokedeCsv.fuyoRow(x.inp); });
        var ftsu=Number(fco.baitaiTsuban||0);
        var ff=TodokedeCsv.fuyoCsv({ jimusho:fi[0].inp.jimusho,
          baitai:{ tsuban:TodokedeCsv.nextTsuban(ftsu), ymd:new Date().toISOString().slice(0,10) },
          kyou:new Date().toISOString().slice(0,10), rows:frows });
        /* ★1件でも 合わなければ 1バイトも 作らない★（門は 共通の CHECK）
           ★ここで 止めないと 年金機構に 出してから 弾かれる＝直しが 一番 高くつく★ */
        if(ff.kensa && ff.kensa.errors.length){
          uiAlert('年金機構の 決まりに 合わない所が '+ff.kensa.errors.length+'件 あります。'
            +String.fromCharCode(10)+ff.kensa.errors.slice(0,5).map(function(x){
              return (x.gyo)+'枚目 項番'+x.no+' '+x.name+'／'+x.why; }).join(String.fromCharCode(10)));
          return;
        }
        /* ★未測定を 緑で 返さない★＝様式を 見ていない行が 1本でも 在れば 出さない */
        if(ff.kensa && ff.kensa.mihakari){
          uiAlert('この届出の 検査が できていない行が '+ff.kensa.mihakari+'件 あります。出しません。'); return;
        }
        if(!ff.bytes.length){ uiAlert('出せる 届書が ありませんでした（1バイトも作っていません）。'); return; }
        if(ff.tooBig){
          uiAlert('このファイルは 4.5MB 以上になるため、電子申請できません。'
            +String.fromCharCode(10)+'人数を分けて 出してください（日本年金機構の決まりです）。');
          return;
        }
        dlBytes(ff.bytes, ff.name, 'text/csv');
        state.company.baitaiTsuban=TodokedeCsv.nextTsuban(ftsu);
        persistSaveDebounced();
        return;
      }
      if(e.target.closest('#b-shutoku-csv')){
        ...
        if(!kr.length){ uiAlert('資格取得の 対象者が いません（従業員マスタの 入社日を 入れてください）。'); return; }
        var ki=kr.filter(function(x){ return TodokedeCsv.dasuKaShutoku(shutokuCsvInput(x)); });
        if(!ki.length){ uiAlert('電子申請に 出せる人が いませんでした。'+String.fromCharCode(10)
          +'入社日・性別・郵便番号・住所（カナ）が 要ります（マイナンバーを お預かりしない為）。'); return; }
        var kco=state.company||{};
        var krows=ki.map(function(x){ return TodokedeCsv.shutokuRow(shutokuCsvInput(x)); });
        var ktsu=Number(kco.baitaiTsuban||0);
        var kf=TodokedeCsv.shutokuCsv({ jimusho:shutokuCsvInput(ki[0]).jimusho,
          baitai:{ tsuban:TodokedeCsv.nextTsuban(ktsu), ymd:new Date().toISOString().slice(0,10) },
          kyou:new Date().toISOString().slice(0,10), rows:krows });
        if(kf.kensa && kf.kensa.errors.length){
          uiAlert('年金機構の 決まりに 合わない所が '+kf.kensa.errors.length+'件 あります。'
            +String.fromCharCode(10)+kf.kensa.errors.slice(0,5).map(function(x){
              return (x.gyo)+'人目 項番'+x.no+' '+x.name+'／'+x.why; }).join(String.fromCharCode(10)));
          return;
        }
        if(!kf.bytes.length){ uiAlert('出せる人がいませんでした（1バイトも作っていません）。'); return; }
        if(kf.tooBig){
          uiAlert('このファイルは 4.5MB 以上になるため、電子申請できません。'
            +String.fromCharCode(10)+'人数を分けて 出してください（日本年金機構の決まりです）。');
          return;
        }
        dlBytes(kf.bytes, kf.name, 'text/csv');
        state.company.baitaiTsuban=TodokedeCsv.nextTsuban(ktsu);
        persistSaveDebounced();
        return;
      }
      if(e.target.closest('#b-gekkaku-csv')){
        ...
        var gi=gr.filter(function(x){ return TodokedeCsv.dasuKaGekkaku(gekkakuCsvInput(x)); });
        if(!gi.length){ uiAlert('電子申請に 出せる人が いませんでした。'+String.fromCharCode(10)
          +'3か月とも 支払基礎日数が 17日以上（短時間労働者は 11日以上）である 必要が あります。'); return; }
        var gco=state.company||{};
        var grows=gi.map(function(x){ return TodokedeCsv.gekkakuRow(gekkakuCsvInput(x)); });
        ...
        dlBytes(gf.bytes, gf.name, 'text/csv');
        state.company.baitaiTsuban=TodokedeCsv.nextTsuban(gtsu);
        persistSaveDebounced();
        return;
      }
      if(e.target.closest('#b-santei-csv')){
        var yr2=parseInt(String(state.month||'').slice(0,4),10)||2026;
      ...
      if(e.target.closest('[data-fix-bonus-payday]')){
        state.printMode='bonus';
        showScreen('scr-input');
        setTimeout(function(){ var el=$('[data-bn="payDay"]'); if(el && el.offsetParent){ el.scrollIntoView({block:'center'}); el.focus(); } },80);
        return;
      }
      if(e.target.closest('[data-fix-payday]')){
        showScreen('scr-settings');
        var cb=$('#set-seg .seg-b[data-set="company"]'); if(cb)cb.click();
        /* ★何もしないcatchを 足さない★（見張りが 46→47 で 捕まえた・2026-09-03）＝
           見えている時だけ 指を 当てる（見えない物に focus しない）。 */
        setTimeout(function(){ var el=$('#c-payday-day'); if(el && el.offsetParent){ el.scrollIntoView({block:'center'}); el.focus(); } },60);
        return;
      }
      ...
      /* ★押せる案内の 受け口は ここ 1つ★（2026-09-03 穴①）。
         data-fix-emp（番号）＝最低賃金割れ／data-fix-emp-id（id）＝県未選択。
         ★id で 来た物は 番号に 直してから★ 同じ道を 通す＝★2つ目の 仕組みを 作らない★。
         ★id で 指す方が 正しい★（番号は 並び替え・絞り込みで 別人を 指す）。 */
      var fe=e.target.closest('[data-fix-emp]');
      if(!fe){
        var ft=e.target.closest('[data-fix-emp-id]');
    ...
    var kp=$('#c-pref');
    if(kp) kp.addEventListener('change',function(){
      state.company.pref=this.value; persistSaveDebounced();
  ...
  function empKesu(i){
    ...
    el.addEventListener('change',function(ev){
      ...
      /* ★家族（被扶養者）の 打ち込み★（2026-09-08）
         印は data-kz="従業員の番号:家族の番号:欄の名前"。
         ★同居／別居は 1/0 の 字で 来る★ので 真偽に 直して しまう（lib が そう 受け取る）。 */
      var kzf=ev.target.closest('[data-kz]');
      if(kzf){ var kp=String(kzf.dataset.kz).split(':'); var ke=state.employees[+kp[0]];
        if(ke&&ke.kazoku&&ke.kazoku[+kp[1]]){ var fld=kp[2], v=ev.target.value;
      ...
      /* ★県は その場で 追いつかせる★（2026-09-03 穴②）＝警告の箱と 社会保険の 自動計算だけ 更新。
         丸ごと 描き直すと ★開いていた 札が 閉じる★ので しない。 */
      if(f==='pref'){ refreshEmpWarn(); refreshShaho(i); }
    ...
    (function(){ var fb=$('#furi-box'); if(!fb) return;
      ...
      /* ★data-fc は 振込の画面にだけ 繋がっていた★（2026-09-03 実測＝算定基礎の 4問が 保存されず
         「入れたのに まだ 出せません」のままだった）。★帳票の 画面にも 同じ 受け口を 付ける★
         ＝★同じ 仕組みを 使い回す（2つ目を 作らない）★。 */
      var choHost=$('#view-cho');
      if(choHost){ choHost.addEventListener('input', setFc); choHost.addEventListener('change', setFc); }
    ...
    Store.listMeisaiPub(rosterIds()).catch(function(e){ toast('公開ずみの明細を読み込めませんでした（' + ((e&&e.message)||'つながりません') + '）。★0件ではありません★'); throw e; }).then(function(list){
      ...
      host.innerHTML='<p class="hint" style="margin:-4px 0 10px">従業員に<b>リンク（QR）</b>を最初に一度だけ渡してください（LINE/メール/手渡し）。<b>リンクが鍵</b>なので本人にだけ。従業員は開いて<b>自分のパスワードを決めるだけ</b>→以後はパスワードだけ。<b>「今月を確定」すると、その月は自動でここに公開</b>され、従業員はいつでもどの月でも見られます（毎回「Web明細で公開」を押す必要はありません）。'+(unread?'<b style="color:#92500A"> 未読 '+unread+'件</b>':' <b style="color:#2E7D54">全員が閲覧済み</b>')+'</p>'
        ...
        +list.map(function(p){
          ...
          var pwState=p.hasPassword?'<span style="font-size:10.5px;color:#333333">パスワード設定済</span>':'<span style="font-size:10.5px;color:#92500A">パスワード未設定</span>';
          var consentState=p.consentAt?'<span style="font-size:10.5px;color:#333333">・同意済</span>':'<span style="font-size:10.5px;color:#92500A">・未同意</span>';
          ...
          return '<div style="border:1px solid #d4eae0;border-radius:10px;padding:9px 11px;margin-bottom:7px">'
            ...
            +'<div style="display:flex;gap:6px;align-items:center;margin-top:5px"><span style="font-size:11px;color:#555555;min-width:52px">リンク</span><input class="finput" readonly value="'+attr(handoutUrl)+'" style="flex:1;font-size:11px;padding:7px 9px" onclick="this.select()"><button class="btn-ghost wm-copy" data-link="'+attr(handoutUrl)+'" style="padding:7px 10px;font-size:11px">コピー</button><button class="btn-ghost wm-qr" data-qr-url="'+attr(handoutUrl)+'" data-qr-name="'+attr(p.name||'')+'" style="padding:7px 10px;font-size:11px">QR</button></div>'
            +codeRow
