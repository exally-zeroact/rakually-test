...
T('★何が出せるかの表＝「出せる」を実物から出している（手書きでない）', () => {
  ...
  /* ★ 2026-09-08 5件→6件★（被扶養者(異動)届 2202700 を 足した）
     土台だけでは なく ★家族を 入れる 画面と 保管と 見張り★が 揃った日に 上げた。
     実ブラウザで SHFD0006.CSV を 落として 139項目を 数えている。 */
  eq(dekiru.length, 6, '★出せるのが ' + dekiru.length + '件★（算定・月変・賞与・資格取得・資格喪失・被扶養者 の 6件 のはず）');
  ok(dekiru.some((x) => /被扶養者/.test(x.nm)), '★被扶養者(異動)届が「出せます」に なっていない★');
  ...
  /* ★「まだ」の 理由は 2つに 分かれる★ */
  /* ★uchi（うちが 作っていない）は 2026-09-08 に 0本に なった★
     ＝最後の 1本だった 被扶養者(異動)届が 出せるように なった。
     ★段そのものは 残す★（azukari と 同じ扱い）＝また 同じ形が 出た時に すぐ 使える。
     ★残る「まだ」は 全部 nenkin★（年金機構の 検査が 対応していない）。 */
  eq(mada.filter((x) => x.riyu === 'uchi').length, 0,
    '★うちが 作っていない 物が 残っている★（被扶養者届を 作ったので 0本のはず）');
