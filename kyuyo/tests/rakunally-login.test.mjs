...
//   本番の js/rakunally-login.js には意図的に未展開。統合時に本番のログイン挙動を変えないため、
//   ルートの部品は本番版のまま据え置いた。よってこのテストは対象機能が無く成立しない。
//   ★item C を本番へ展開する判断が出たら、この注記を外してCIに戻すこと（テストは消していない）。
// rakunally-login.test.mjs — ★item C: メール確認ON前提のsignup分岐★
//  共通ログイン部品(js/rakunally-login.js)の signup を検証:
...
//   - 通常loginは従来どおり
//  依存: jsdom。使い方: node tests/rakunally-login.test.mjs (jsdom未導入ならSKIP)
...
function loadLogin() {
  ...
  el.textContent = fs.readFileSync(path.join(ROOT, '..', 'js/rakunally-login.js'), 'utf8');
...
runs.push(T('確認OFF: signupでsession有り→即ログイン(onLogin呼ぶ・待機画面出さない)', async function () {
  ...
  let loggedIn = null;
  const L = win.RakunallyLogin.mount({ app: '給料明細', sb, onLogin: function (u) { loggedIn = u; } });
...
runs.push(T('待機画面→戻るで通常ログインへ復帰(btnLogin/btnSignupが再び出る)', async function () {
  const win = loadLogin();
  const sb = makeSb({ signUp: { data: { user: { email: 'new@b.com' }, session: null } } });
  const L = win.RakunallyLogin.mount({ app: '給料明細', sb, onLogin: function () {} });
  L.show(); fill(win, 'new@b.com', 'secret1');
  await clickAsync(win, 'btnSignup');
