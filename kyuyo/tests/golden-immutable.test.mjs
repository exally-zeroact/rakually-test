 ...
 *
 * ★2026-09-08★ ★kazoku（家族・被扶養者の一覧）★を 足した。
 *   司さん「★出来てないものは全部やれや★」＝残っていた 最後の 1本
 *   ★被扶養者(異動)届（様式2202700・139項目）★は 人数では 出せない。
 *   氏名・生年月日・続柄・住所…を 1人ずつ 預かる必要が ある。
 *   ★期待値(datasets)は1バイトも変えていない★（kazoku は お金の計算に 入らない）。
 *   ★マイナンバーは 相変わらず 預からない★（届出は 住所で 出す）。
 *
 * ★2026-09-05（同じ日の 3回目）★ ★kisoNenkin（基礎年金番号）★を 足した。
 *   司さん「★競合がやりよることはやれ★」＝マネーフォワードと同じく お預かりする。
 *   ★退社の 届出（資格喪失届 2201700）は 原文 項番12 で 個人番号か 基礎年金番号が 必須★。
 *   ★期待値(datasets)は1バイトも変えていない★／ops-golden-parity 59名・差分0件。
 *
 * ★2026-09-05（同じ日の 2回目）★ さらに ★kana（氏名カナ）★を 足した。
 *   ★実ブラウザで お客さんの道を 通したら 入れる欄が 1つも 無かった★（kyuyo/tests/shutoku-ui.mjs）。
 *   4つの 届出 ぜんぶで 必須（算定6／月変6／賞与6／取得7）なのに ★誰も 入れられなかった★。
 *   ★期待値(datasets)は1バイトも変えていない★／ops-golden-parity 59名・差分0件。
 *
