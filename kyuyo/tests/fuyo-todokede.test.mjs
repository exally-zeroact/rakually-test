...
/* ── ⑧ ★画面の 続柄の コードが 検める側と 一致するか★（2026-09-14） ──
   司さんの 指摘で ★言い方（ラベル）だけ★ 変えた（02「それ以外の子」→「子（それ以外）」）。
   ★言い方を 触る時に 一番 こわいのは コードが ずれる事★＝
   2026-09-08 に ★続柄コードを 12個 とも 間違えた★前科が 在る（03 は「子」では ない）。
   ⇒ ★画面の 表と 検める側の 一覧を 突き合わせる★（どちらかを 触ったら 赤）。 */
t('⑧ ★画面の 続柄コードが 検める側と 1つも ずれていない★', () => {
  const APP = require_('node:fs').readFileSync(path.join(ROOT, 'kyuyo/js/app.js'), 'utf8');
  const m = APP.match(/var ZOKU = \[[\s\S]*?\];/);
  ok(m, '★画面の 続柄の 表が 見つからない★');
  const codes = [...m[0].matchAll(/\['([0-9]{2})',/g)].map((x) => x[1]);
  eq(JSON.stringify(codes), JSON.stringify(C.FUYO_ZOKU), '画面と 検める側で 続柄コードが 違う');
  /* ★配偶者は 別扱い★＝この表では コードでは なく haigusha という 印で 持つ */
  ok(m[0].indexOf("'haigusha'") > 0, '★配偶者の 行が 消えた★');
});
